 ...
 * <code>WorkspaceSubscriber</code>s to which it sends notifications of
...
public class WorkspacePublisherAdaptor implements WorkspacePublisher
{
	...
	/**
	 * Creates a workspace publisher with an empty list of subscribers.
	 */
	...
	{
		fireRepaintRequired(new WorkspaceMessage(
				WorkspaceMessage.DISPLAY,
				null,
	...
	/**
	 * Sends a notification to subscribers of changes to the display options
	 * such as Zoom, or toggling show grid, LaTeX rendering, UniformNode size
	 * etc. Passes the given message to each subscriber.
	 * 
	 * @param message
	 ...
	 * (added), closed (removed) or renamed.
	 * 
	 * @param message
	 */
	...
	/**
	 * Sends notification to subscribers of changes the workspace. Intended to
	 * facilitate changes to the current set of interface tools (a.k.a. work
	 * bench) needed to view and manipulate different kinds of DES models.
	 * 
	 * @param message
	 */
	...
	/*
	 * protected void fireSaved() { for( WorkspaceSubscriber s : subscribers ) {
	 * s.workspaceSaved(); } }
	 */
}
