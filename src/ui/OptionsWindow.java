 ...
 * {@link main.Hub#registerOptionsPane(OptionsPane)} and providing an
 * {@link pluggable.ui.OptionsPane} as an argument. The set of controls for the
 * module or plugin have to appear in the {@link javax.swing.JPanel} that is
 * returned by the {@link pluggable.ui.OptionsPane#getPane()} method.
 * 
...
public class OptionsWindow extends EscapeDialog
{
	 ...
	 * buttons displayed underneath a selected {@link pluggable.ui.OptionsPane}.
	 */
	protected JPanel resetCommitPane;

	/**
	 * Creates and displays the Options dialog, where the last selected section
	 * is pre-selected.
	 * 
	 ...
	 * 
	 * @param openTitle
	 *            title of the section to be pre-selected
	 * @see #OptionsWindow()
	 */
	public OptionsWindow(String openTitle)
	{
		...
		for (Enumeration<String> i = optionsRegistry.keys(); i
				.hasMoreElements();)
		{
		...
		sectionList.setPreferredSize(new Dimension(LISTWIDTH, sectionList
				.getPreferredSize().height));
		sectionList.setMinimumSize(new Dimension(LISTWIDTH, sectionList
				.getPreferredSize().height));
		...
		JSplitPane stuffPane = new JSplitPane(
		...
		{
			if (sectionList
					.getModel().getElementAt(i).toString().equals(openTitle))
			{
				sectionList.setSelectedIndex(i);
				break;
			}
		}
		if (sectionList.isSelectionEmpty()
				&& sectionList.getModel().getSize() > 0)
		{
	 ...
	 * <p>
	 * Called when the user presses the <code>Escape</code> key to close the
	 * Options dialog. Also called by the Cancel button and when the user closes
	 * the window manually.
	 */
	@Override
	 ...
	 * {@link main.Hub#registerOptionsPane(OptionsPane)} to register option
	 * panes.
	 * 
	 * @param pane
	 *            the {@link pluggable.ui.OptionsPane} which has to be added.
	 * @see main.Hub#registerOptionsPane(OptionsPane)
	 */
	 ...
	 * 
	 * @param title
	 *            the title of the selected section
	 */
	protected void loadOptionsFor(String title)
	{
		...
		optionsHolder.add(new Box.Filler(new Dimension(0, 0), new Dimension(
				0,
				20), new Dimension(0, 10)));
	...
	{
		optionsRegistry
				.get(sectionList.getSelectedValue().toString()).resetOptions();
	}

	/**
	 * Commits the values of the controls of the currently selected options
	 * pane.
	 */
	protected void commitPane()
	{
		optionsRegistry
				.get(sectionList.getSelectedValue().toString()).commitOptions();
	}
