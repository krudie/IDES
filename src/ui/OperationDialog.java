...
public class OperationDialog extends EscapeDialog
{
	...
	public OperationDialog()
	{
		...
		spd.setBorder(BorderFactory.createTitledBorder(Hub
	...
	protected void fillInOutputs()
	{
		...
		if (outputNames.size() == 1)
		{
			...
			{
				if (lastSetNames.firstElement().equals(outputNames
		...
		else
		{
			for (int i = 0; i < outputNames.size(); ++i)
			{
				...
				{
					if (lastSetNames.elementAt(i).equals(outputNames
							.elementAt(i).getText()))
					{
	...
	private List<Object> getSelectedItems(int inputIdx)
	{
		...
		{
			values = ((ItemSelectable)inputs.elementAt(inputIdx))
					.getSelectedObjects();
		}
		...
		else if (inputs.elementAt(inputIdx) instanceof JTextField)
		{
			...
			{
				values = new Object[] { ((JTextField)inputs.elementAt(inputIdx))
						.getText() };
			}
		}
	...
	protected JComponent getSingleSelector(Class<?> type)
	{
		...
		else if (Double.class.equals(type) || Long.class.equals(type)
				|| String.class.equals(type))
		{
			JTextField tf = new JTextField();
			tf.setMaximumSize(new Dimension(tf.getMaximumSize().width, tf
					.getPreferredSize().height));
		...
		selector.setMaximumSize(new Dimension(
				selector.getMaximumSize().width,
				selector.getPreferredSize().height));
	...
	private static class NameCellRenderer extends JLabel implements
			ListCellRenderer
	{
	...
	ActionListener computeAction = new ActionListener()
	{
		public void actionPerformed(ActionEvent ae)
		{
			...
			else
			{
				...
				Operation op = OperationManager.instance().getOperation(opList
				...
				for (int i = 0; i < inputs.size(); ++i)
				{
					...
					if (DESEventSet.class.equals(op.getTypeOfInputs()[i]))
					{
						...
						{
							eventSet
									.add(((GlobalEventList.EventRecord)item).event);
						}
						items = new LinkedList<Object>();
						items.add(eventSet);
					}
					else if (items.size() == 1
							&& items.get(0) instanceof String)
					{
						try
						{
							if (Double.class.equals(op.getTypeOfInputs()[i]))
							{
								items.set(0, Double.valueOf((String)items
										.get(0)));
							}
							else if (Long.class.equals(op.getTypeOfInputs()[i]))
							{
								items
										.set(0, Long.valueOf((String)items
												.get(0)));
							}
						}
						catch (NumberFormatException e)
						{
							Hub.displayAlert(Hub
									.string("cantInterpretInputNumber"));
							return;
						}
					}
