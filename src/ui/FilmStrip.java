...
public class FilmStrip extends JPanel implements WorkspaceSubscriber,
		MouseListener, MouseMotionListener, DESModelSubscriber
{
	...
	private static final Border SELECTED_BORDER = BorderFactory
			.createLineBorder(UIManager
					.getColor("InternalFrame.borderDarkShadow"), 3);
	...
	private void buildThumbnailBoxes()
	{
		...
		if (selectedIdx > -1)
		{
			...
			scrollRectToVisible(new Rectangle((selectedIdx)
					* (THUMBNAIL_SIZE + SPACER_SIZE), 0, THUMBNAIL_SIZE, 0));
		}
	}

	/**
	 * Gets all graph models from the workspace and renders each in its own
	 * view.
	 */
	...
	/**
	 * Figure out which graph was selected, and set it as the currently active
	...
	{
		/*
		 * This needs to be handled here for usability purposes: testing shows
		 * users are prone to initializing a mouse dragging event when clicking
		 * on a thumbnail to switch to a new model. -- Lenko
		 */
		mouseClicked(arg0);
	}
	 ...
	 * activates the closeButton in the corner of the thumbnail, and ensures
	 * that at most one closeButton is visible at any given time. We're still
	 * having issues with some of the mouse motion events being lost to
