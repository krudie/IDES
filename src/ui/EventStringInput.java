...
public class EventStringInput extends JPanel
{
	...
	protected static class EventStringChooser extends EscapeDialog
	{
		...
		{
			super(JOptionPane.getFrameForComponent(input),
					"Select events to form string",
					true);
			parent = input;
			...
			DocumentListener al = new DocumentListener()
			{
				...
				private void configStuff(String symbol)
				{
					...
					if (!listAvailableEvents.existsElement(symbol))
					{
						...
						createBox.setBorder(BorderFactory
								.createTitledBorder(BorderFactory
										.createLineBorder(UIManager
					...
					else
					{
						...
						createBox
								.setBorder(BorderFactory
										.createTitledBorder(BorderFactory
												.createLineBorder((textField
														.getText().equals("") ? UIManager
														.getColor("TextField.shadow")
														: UIManager
			...
			textField.addKeyListener(new KeyAdapter()
			{
				...
				public void keyReleased(KeyEvent ke)
				{
					...
					int i = listAvailableEvents
							.indexOfFirstElementWithPrefix(symbol);
					if (i > -1)
					{
			...
			buttonCreate.setPreferredSize(new Dimension(
					buttonCreate.getPreferredSize().width,
					textField.getPreferredSize().height));
			...
			createBox.setBorder(BorderFactory
					.createTitledBorder(BorderFactory
							.createLineBorder(UIManager
									.getColor("TextField.shadow"), 1), Hub
							.string("concatToString")));
			...
			listAvailableEvents
					...
					.addListSelectionListener(new ListSelectionListener()
					{
						public void valueChanged(ListSelectionEvent e)
						{
							...
							{
								listAssignedEvents.clearSelection();
								Object o = listAvailableEvents
										.getSelectedValue();
								if (o != null)
								{
									selectedEvent = (DESEvent)o;
									listAvailableEvents.setSelectedValue(o,
											true);
									if (listAvailableEvents.hasFocus())
									{
										textField.setText(selectedEvent
												.getSymbol());
									}
									if (inserted
											&& textField.getSelectedText() == null
											&& textField.getCaretPosition() == textField
													.getText().length())
									{
										String originalText = textField
												.getText();
										textField.setText(selectedEvent
												.getSymbol());
										textField.setSelectionEnd(textField
												.getText().length());
										textField
												.setSelectionStart(originalText
														.length());
									}
			...
			pane.setBorder(BorderFactory
					.createTitledBorder("Events in workspace"));
			...
			buttonOK
					.setPreferredSize(new Dimension(Math.max(buttonOK
							.getWidth(), buttonCancel.getWidth()), buttonOK
							.getHeight()));
			buttonOK.invalidate();
			buttonCancel.setPreferredSize(new Dimension(Math.max(buttonOK
					.getWidth(), buttonCancel.getWidth()), buttonCancel
					.getHeight()));
		...
		public void update()
		{
			...
			for (Iterator<DESModel> models = Hub.getWorkspace().getModels(); models
					.hasNext();)
			{
		...
		private class MutableList extends JList
		{
			...
			/**
			 * @param symbol
			 * @return index of the first element found with prefix of string
			 *         representation matching symbol, -1 if no such element
			 */
		 ...
		 * @author Squirrel
		 */
		private class AddButtonListener extends MouseAdapter implements
				ActionListener
		{
		 ...
		 * @author Squirrel
		 */
		private class RemoveButtonListener extends MouseAdapter implements
				ActionListener
		{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				DESEvent selected = (DESEvent)listAssignedEvents
						.getSelectedValue();
				if (selected != null)
				{
		...
		protected Action createListener = new AbstractAction()
		{
			...
			public void actionPerformed(ActionEvent actionEvent)
			{
				...
				listAvailableEvents.setSelectedIndex(listAvailableEvents
						.indexOfFirstElementWithPrefix(textField.getText()));

				new AddButtonListener().actionPerformed(new ActionEvent(
						this,
						0,
						""));
				textField.setText("");
		...
		private class CommitListener implements ActionListener
		{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				...
				if (!listAvailableEvents.existsElement(textField.getText())
