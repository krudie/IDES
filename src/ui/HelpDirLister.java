...
public class HelpDirLister extends TreeMap<String, String>
{
	...
	public static void init()
	{
		...
		try
		{
			...
			helpDir = new File(helpDir.getAbsolutePath() + File.separator
					+ "help");
			if (!helpDir.exists())
			{
				helpDir = null;
		...
		File localHelpDir = new File((String)System
		...
		if (helpDir != null)
		{
			try
			{
				for (File f : helpDir.listFiles())
				{
					if (f.isDirectory())
					{
						File index = new File(f.getAbsolutePath()
