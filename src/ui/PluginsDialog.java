...
public class PluginsDialog extends EscapeDialog
{
	...
	protected class DescriptionBox extends JPanel
	{
		...
		protected class ViewLicenseButton extends JButton implements
				ActionListener
		{
			...
			{
				dialog = new EscapeDialog(Hub.getMainWindow(), Hub
						.string("license")
						+ " ("
						+ plugin.getName()
						+ " "
				...
				JTextArea license = new ContractableTextArea(plugin
						.getLicense());
				...
				dialog.pack();
				dialog.setLocation(Hub.getCenteredLocationForDialog(dialog
						.getSize()));
		...
		public DescriptionBox(Plugin plugin)
		{
			...
			creditBox.add(new JLabel(Hub.string("credits") + ": "
					+ plugin.getCredits()));
