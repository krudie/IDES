...
public class FSAStateLabeller
{
	...
	/**
	 * Labels in a human-readable way the states in models obtained by composing
	 * a number of other models (e.g., by calling the "product" or "sync"
	 * operations).
	 * <p>
	 ...
	 * 
	 * @param fsa
	 *            the model whose states have to be labelled.
	 */
		...
		if (((String[])fsa.getAnnotation(AnnotationKeys.COMPOSED_OF)).length > 1)
		{
			...
			for (Iterator<FSAState> si = fsa.getStateIterator(); si.hasNext();)
			{
				...
				else
				{
					...
					subState = gs[gs.length - 1]
							.getState(composition[gs.length - 1]);
					if (subState == null)
					{
						nameError(s);
						continue;
			...
			for (Iterator<FSAState> si = fsa.getStateIterator(); si.hasNext();)
			{
				...
				else
				{
					...
					if (composition.length > 1)
					{
						...
						subState = g
								.getState(composition[composition.length - 1]);
						if (subState == null)
						{
							nameError(s);
							continue;
