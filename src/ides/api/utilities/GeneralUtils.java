...
public class GeneralUtils
{
	 ...
	 * <p>
	 * Normally, a {@link JOptionPane} has mnemonic key bindings with its
	 * buttons. For example, in order to select the Yes button via the keyboard,
	 * one needs to press <code>Alt+Y</code>. However, this may be
	 * counter-intuitive as many users would expect to use simply the key
	 * <code>Y</code>.
	 * <p>
	 * This class provides a hack which lets the use of simple key shortcuts
	 * instead of the mnemonic shortcuts. It does the following:
	 * <ol>
	 * <li>Creates a {@link JComponent} with the given text message.
	 * <li>Checks what are the default mnemonic shortcuts. In the default
	 * English Java installation these should be <code>Alt+Y</code> for the Yes
	 * button, <code>Alt+N</code> for the No button and no shortcuts for the OK
	 * and Cancel buttons.
	 ...
	 * <li>For each one of the shortcuts, an {@link Action} is created which
	 * looks up the corresponding button in the {@link JOptionPane} and calls
	 * {@link JButton#doClick()} on it.
	 * </ol>
	 * All of this is necessary since the buttons of a {@link JOptionPane}
	 * cannot be accessed and customized directly. More information can be found
	 * on <a href="http://forums.sun.com/thread.jspa?threadID=5335863">this
	 * page</a>.
	 * <p>
	...
	public static class JOptionPaneKeyBinder
	{
		...
		/**
		 * Wraps a message in a {@link JComponent} with simplified key bindings
		 * for the {@link JOptionPane} buttons. Use the component instead of the
		 * string message when calling option pane methods.
		 * <p>
		 * For example<br>
		 * <code>JOptionPane.showConfirmDialog(window,message,title,JOptionPane.YES_NO_CANCEL_OPTION);</code>
		 * <br>
		...
		public static JComponent messageLabel(String message)
		{
			...
			InputMap inputMap = label
					.getInputMap(JLabel.WHEN_IN_FOCUSED_WINDOW);
			...
			actionMap.put(YES, new ShortcutHandler(UIManager
					.getDefaults()
					.getString("OptionPane." + YES + "ButtonText")));
			actionMap.put(NO,
					new ShortcutHandler(UIManager
							.getDefaults().getString("OptionPane." + NO
									+ "ButtonText")));
			actionMap.put(OK,
					new ShortcutHandler(UIManager
							.getDefaults().getString("OptionPane." + OK
									+ "ButtonText")));
			actionMap.put(CANCEL, new ShortcutHandler(UIManager
					.getDefaults().getString("OptionPane." + CANCEL
							+ "ButtonText")));
			return label;
		}
		 ...
		 * {@link JOptionPane} button. Only one of
		 * {@link JOptionPaneKeyBinder#YES}, {@link JOptionPaneKeyBinder#NO},
		 * {@link JOptionPaneKeyBinder#OK} and
		 * {@link JOptionPaneKeyBinder#CANCEL} is a valid input. If the simple
		 * key cannot be retrieved (e.g., there is no mnemonic shortcut
		 * associated with the button), returns <code>null</code>.
		 * 
		 * @param button
		 *            one of {@link JOptionPaneKeyBinder#YES},
		 ...
		 *         {@link JOptionPane} button, or <code>null</code> if there is
		 *         no such key
		 */
		protected static String getMnemonicKey(String button)
		{
			...
			try
			{
				code = Integer.parseInt(UIManager
						.getDefaults().getString("OptionPane." + button
								+ "ButtonMnemonic"));
				if (code == 0)
				{
					return null;
		...
		protected static class ShortcutHandler extends AbstractAction
		{
			...
			/**
			 * Create a handler which will activate the button with the given
			 * text.
			 * 
			 * @param buttonText
			 *            the text of the button which needs to be activated
			 */
			 ...
			 * {@link JButton#doClick()} method. The button is looked up in the
			 * top-level ancestor of the source (component) of this action.
			 */
			public void actionPerformed(ActionEvent arg0)
			{
				clickButton(((JComponent)arg0.getSource())
						.getTopLevelAncestor(), buttonText);
			}
		}

		/**
		 * Calls the {@link JButton#doClick()} method on the button with the
		 * given text in the given container. If the button cannot be found in
		 ...
		 * 
		 * @param container
		 *            the container which contains the button
		 * @param buttonText
		 *            the text of the button
		 * @return the button with the text or <code>null</code> if the button
		 *         cannot be found
		 */
		private static JButton findButton(Container container, String buttonText)
		{
			for (Component component : container.getComponents())
			{
				...
				{
					JButton button = findButton((Container)component,
							buttonText);
					if (button != null)
					{
						return button;
