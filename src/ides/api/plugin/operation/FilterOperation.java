...
public interface FilterOperation extends Operation
{
	 ...
	 * accepted by this method have to be of the same number, type and semantics
	 * as the inputs for {@link Operation#perform(Object[])}. The outputs
	 * produced by this method have to be of the same number, type and semantics
	 * as the outputs from {@link Operation#perform(Object[])}. Since some
	 * inputs may serve as outputs, they have to be included in the array with
	 * outputs.
	 * 
	 * @param inputs
	 *            inputs to the operation (must conform to the types described
	 *            in {@link Operation#getTypeOfInputs()})
	 * @return outputs of the operation (must conform to the types described in
	 *         {@link Operation#getTypeOfOutputs()}). The inputs which were
	 *         modified have to appear in the array of outputs.
