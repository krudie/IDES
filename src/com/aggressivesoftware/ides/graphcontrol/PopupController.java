...
/**
 * This class handles all popup menus for the GraphController.
 * Note that while many menus are in sync with the menu-controller, any menu can break free
 * and behave in entirely cusom ways.
 * 
 * @author MichaelWood
 */
...
{
	/**
     * The platform in which this PopupController exists.
     */
	...
	private void initializeNodePopup()
	{
		...
		mitm_node_startstate.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
				...
				{ 
					if (gp.gc.menued_object != null)
					{
						...
						{ 
							if (gp.gc.start_state_node != null) { gp.gc.start_state_node.removeAttribute(GraphObject.START_STATE); }
		...
		mitm_node_markedstate.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
				...
				{ 
					if (gp.gc.menued_object != null)
					{
						if (mitm_node_markedstate.getSelection()) { gp.gc.menued_object.addAttribute(GraphObject.MARKED_STATE); }
						else { gp.gc.menued_object.removeAttribute(GraphObject.MARKED_STATE); }
			...
			{ 
				...
				{ 
					...
					{	
						...
						{ gp.gc.floating_text.initialize(((Node)gp.gc.menued_object).origin(), ((Node)gp.gc.menued_object).origin(), ((Node)gp.gc.menued_object).latex_label); }
						else
						{ gp.gc.floating_text.initialize(((Node)gp.gc.menued_object).origin(), ((Node)gp.gc.menued_object).origin(), ((Node)gp.gc.menued_object).glyph_label); }									
	...
	private void initializeEdgePopup()
	{
			...
			{ 
				...
				{ 
					if (gp.gc.menued_object != null)
					{
						...
						{ gp.tab_folder.setSelection(GraphingPlatform.SPECIFICATIONS_TAB); }
					}
				} 
			}
		);

		mitm_edge_reset.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
		...
		mitm_edge_straighten.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
	...
	private void initializeAreaPopup()
	{
		...
		mitm_internal_snap.setText(gp.rm.getString("internal_snap.mtext"));
		MenuItem mitm_internal_reset = new MenuItem(mnu_internal, SWT.PUSH); 
		mitm_internal_reset.setText(gp.rm.getString("internal_reset.mtext"));
		...
		mnu_internal.addMenuListener ( new MenuListener() { public void menuHidden(MenuEvent e) { gp.gc.j2dcanvas.setMenu(null); } public void menuShown(MenuEvent e) { } } );
		
		mitm_internal_reset.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
		...
		mitm_internal_snap.addListener
		(
			SWT.Selection, 
			new Listener() 
			{ 
		...
		gp.mc.edit_undo.addPopupMitm(mnu_external);		
		gp.mc.edit_redo.addPopupMitm(mnu_external);		
		mitm_sperator = new MenuItem(mnu_external, SWT.SEPARATOR); 
		MenuItem mitm_external_paste = gp.mc.edit_paste.addPopupMitm(mnu_external, true, false);		
		mnu_external.addMenuListener ( new MenuListener() { public void menuHidden(MenuEvent e) { gp.gc.j2dcanvas.setMenu(null); } public void menuShown(MenuEvent e) { } } );
		
		// note that we provide a cusotm Adapter instead of using the default provided by the UnifiedMenu
		mitm_external_paste.addSelectionListener
		(
