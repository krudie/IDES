...
/**
 * A Curve is a Bezier or Cubic curve, with the concept of a starting end (tail) and a finishing edn (head).
 * The curve is aware of the node at its tail, the node at it's head, and the ArrowHead at its head.
 * It is only indirectly aware of the GraphModel, and the Edge it represents.
 * This class primarly handles the geometric concepts of the Edge.
 * 
 * @author Michael Wood
 */
public class Curve 
{
	...
	/**
     * Remembers the angles between the node origins and the 4 curve points,
     * and remembers the radius at which the ctrl points exist from their nodes.
     ...
     * The ctrl_mag_factors are the ctrl_flat_distances divided by the distance between anchors.
     * The ctrl_ratios are the ctrl_flat_distances divided by the ctrl_radial_distances.
     */
	private float tail_anchor_angle = 0,
				  tail_ctrl_angle = 0,
				  tail_ctrl_radius = 0,
				  tail_ctrl_mag_factor = 0,
				  tail_ctrl_ratio = 0,
				  head_ctrl_ratio = 0,
				  head_ctrl_mag_factor = 0,
				  head_ctrl_radius = 0,
				  head_ctrl_angle = 0,
				  head_anchor_angle = 0;	
	 ...
	 */
	/**
	 * Remember the coefficients for the distances between the end points
	 * of the curve and the control points relative to the distance between
	 * the two end points, i.e., D(P1,CTRL1)/D(P1,P2) and D(P2,CTRL2)/D(P1,P2).
	 */
	protected float distanceCoefCtrl1 = 1,
					distanceCoefCtrl2 = 1;
	 ...
	 */
	public Curve(Node tail_node, Node head_node, float x1, float y1, float ctrlx1, float ctrly1, float ctrlx2, float ctrly2, float x2, float y2, UnitVector self_loop_direction)
	{
		this.self_loop_direction = self_loop_direction;
		this.tail_node = tail_node;
		this.head_node = head_node;
	...
	public Curve newClone(Node tail_node, Node head_node)
	{
		...
		if (self_loop_direction != null) { cloned_direction = self_loop_direction.getCopy(); }
		return new Curve(tail_node, head_node, curve.x1, curve.y1, curve.ctrlx1, curve.ctrly1, curve.ctrlx2, curve.ctrly2, curve.x2, curve.y2, cloned_direction);
	}
	 ...
	 */
	/**
	 * Finds the cubic curve parameter t for the intersection of the curve
	 * with a circle that has its centerpoint at one of the endpoints of the curve
	 * (used to find the intersections of the edge with its nodes).
	 *    
	 * @param radius		The radius of the circle.
	 * @param startPoint	true if the circle is at the start point of the
	 * 						curve or false if the circle is at the end point of
	 * 						the curve
	 * @return	the prameter t of the cubic curve which indicates the intersection
	 * 			with the circle
	 */
	 ...
	 *  
	 * @param start		the cubic curve parameter defining the start point 
	 * @param distance	the distance between the points defined by start and the return value 
	 * @param startToEnd	true if distance is computed in the direction of the curve
	 * 						or false if it is computed in the opposite direction
	 * @return			the cubic curve parameter t such that the distance between
	 * 					the points defined by start and t is distance
	 */	
	protected float travelEdge(float start, float distance, boolean startToEnd)
	{
		...
		Point b = calculateBezierPoint(t);
		float d=distance-(float)Math.sqrt((s.x-b.x)*(s.x-b.x)+(s.y-b.y)*(s.y-b.y));
		while(Math.round(d)>0)
		{
			if(startToEnd)
				...
				t-=d/curve.maxLength();
			b = calculateBezierPoint(t);
			d=(float)(distance-Math.sqrt((s.x-b.x)*(s.x-b.x)+(s.y-b.y)*(s.y-b.y)));
		}
		if(t<0)
			t=0;
		else if(t>1)
	...
	/**
	 * Move the tail anchor, and if necessary the tail ctrl point.
	 * The tail anchor is attached to a circle, and the anchor-ctrl line shouldn't breach that circle.
	 * 
	 * @param	origional_configuration	The configuration of the curve at the initiation of movement.
	...
	public void moveTailAnchor(Configuration origional_configuration, Point mouse)
	{
		...
		if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL)
		{
			...
			if (directions.x < 0)
			{
				// only move the ctrl point if it has fallen past the perpendicular of the radial arm.
				float mag = Geometric.distance(origional_configuration.tail_anchor,origional_configuration.tail_ctrl);
				UnitVector d = line.d.newPerpendicular();
				if (directions.y > 0) { d.reverse(); } // make sure we position it in the proper direction.
	...
	/**
	 * Move the tail ctrl, and if necessary the tail anchor point.
	 * The tail anchor is attached to a circle, and the anchor-ctrl line shouldn't breach that circle.
	 * 
	 * @param	origional_configuration	The configuration of the curve at the initiation of movement.
	 * @param	mouse					The current mouse position.
	...
	{
		// update the ctrl point
		curve.ctrlx1 = origional_configuration.tail_ctrl.x + (mouse.x - origional_configuration.movement_origin.x); 
		curve.ctrly1 = origional_configuration.tail_ctrl.y + (mouse.y - origional_configuration.movement_origin.y); 
		...
		if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL)
		{
			...
			if (directions.x < 0)
			{
				// only move the anchor point if the ctrl has been moved past the perpendicular of the radial arm.
				...
				// update the anchor (this forces the location back onto the circle)
	...
	/**
	 * Move the head ctrl, and if necessary the head anchor point.
	 * The head anchor is attached to a circle, and the anchor-ctrl line shouldn't breach that circle.
	 * 
	 * @param	origional_configuration	The configuration of the curve at the initiation of movement.
	 * @param	mouse					The current mouse position.
	 */
	public void moveHeadCtrl(Configuration origional_configuration, Point mouse)
	{
		...
		// update the ctrl point
		curve.ctrlx2 = origional_configuration.head_ctrl.x + (mouse.x - origional_configuration.movement_origin.x); 
		curve.ctrly2 = origional_configuration.head_ctrl.y + (mouse.y - origional_configuration.movement_origin.y); 
	...
	/**
	 * Move the head anchor, and if necessary the head ctrl point.
	 * The head anchor is attached to a circle, and the anchor-ctrl line shouldn't breach that circle.
	 * Also, because of the arrowhead, the anchor should end outside of the circle radius
	 * 
	 * @param	origional_configuration	The configuration of the curve at the initiation of movement.
	 * @param	mouse					The current mouse position.
	 */
	public void moveHeadAnchor(Configuration origional_configuration, Point mouse)
	{
		...
		if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL)
		{
			...
			{
				// only move the ctrl point if it has fallen past the perpendicular of the radial arm.
				float mag = Geometric.distance(origional_configuration.head_anchor,origional_configuration.head_ctrl);
				UnitVector d = line.d.newPerpendicular();
				if (directions.y > 0) { d.reverse(); } // make sure we position it in the proper direction.
    ...
    /**
     * Adjust the curve co-ordinates for a change in the UnitVector or Node position.
     */
	public void recalculateSelfLoop()
	{
		...
		Point p4 = self_loop_direction.newPoint(tail_node.r()+ArrowHead.SHORT_HEAD_LENGTH,tail_node.origin(),-25);
	...
	/**
	 * Move the arrowhead to compensate for curve repositioning.
	 * (tip-anchor, anchor-ctrl, tip-ctrl) forms a triangle.
	 * Use vector addition to find a UnitVector in the tip-ctrl direction.
	 * Move the anchor to a point on that direction HeadLength from the tip.
	 */
	private void updateArrowhead()
	{
		// reposition the arrowhead radially
		UnitVector d = new UnitVector(head_node.origin(),headAnchor());
		Point tip = new Point(head_node.origin().x +head_node.r()*d.x, head_node.origin().y + head_node.r()*d.y);
		d.reverse();
...
//		arrowhead.update(d, tip);
	...
	public void initiateNodeMovement()
	{
		...
		tail_anchor_angle = -Geometric.calculateRadians(on_tail_bisector,tail_node.origin(),tailAnchor());
		tail_ctrl_angle = -Geometric.calculateRadians(on_tail_bisector,tail_node.origin(),tailCtrl());
		tail_ctrl_radius = Geometric.distance(tail_node.origin(),tailCtrl());
		
		// find the fraction of the distance along the bisector direction that this ctrl traverses
		Point tail_ctrl_vector = tailCtrl().minus(tailAnchor());
		float tail_ctrl_flat_mag = Geometric.dotProduct(tail_ctrl_vector,tail_to_head) / (Geometric.magnitude(tail_ctrl_vector)*(float)Math.cos(tail_ctrl_angle));
		...
		head_anchor_angle = -Geometric.calculateRadians(on_head_bisector,head_node.origin(),headAnchor());
		head_ctrl_angle = -Geometric.calculateRadians(on_head_bisector,head_node.origin(),headCtrl());
		head_ctrl_radius = Geometric.distance(head_node.origin(),headCtrl());
		
		// find the fraction of the distance along the bisector direction that this ctrl traverses
		Point head_ctrl_vector = headCtrl().minus(headAnchor());
		float head_ctrl_flat_mag = Geometric.dotProduct(head_ctrl_vector,head_to_tail) / (Geometric.magnitude(head_ctrl_vector)*(float)Math.cos(head_ctrl_angle));
    ...
    /**
     * Recalculate the parameters of this Edge based on a node movement, attempting to approximate the origional layout.
     * 
	 * @param	origional_configuration	The configuration of the Node at the initiation of movement.
	 * @param	mouse					The current mouse position.
     * @param	fixed_circle			The node of the fixed circle.
     * @param	moving_circle			The node of the moving circle.
     */
	public void updateNodeMovement(Configuration origional_configuration, Point mouse, Node fixed_circle, Node moving_circle)
	{				
		// useful directions
		UnitVector tail_to_head = new UnitVector(tail_node.origin(),head_node.origin());
		UnitVector head_to_tail = tail_to_head.newReversed();
		...
		setHeadAnchor(head_node.origin().newPoint(head_anchor_direction,tail_node.r()+ArrowHead.SHORT_HEAD_LENGTH));		

		// new distance between anchors
		float new_anchors_mag = Geometric.distance(tailAnchor(),headAnchor());

		// estimate new ctrl radius'
		float new_tail_ctrl_radius = Geometric.floatBounds((new_anchors_mag*tail_ctrl_mag_factor) / tail_ctrl_ratio);
		float new_head_ctrl_radius = Geometric.floatBounds((new_anchors_mag*head_ctrl_mag_factor) / head_ctrl_ratio);
		
		// this gives bad behaviour if a ctrl radius is greater than the distance between the two nodes.
		float node_distance = Geometric.distance(tail_node.origin(),head_node.origin()) - tail_node.r() - head_node.r();
		...
		if      (moving_circle == tail_node) { tail_radius_adjust = tail_node.r()-origional_configuration.radius; }
		else if (moving_circle == head_node) { head_radius_adjust = head_node.r()-origional_configuration.radius; }
		setTailCtrl(tail_node.origin().newPoint(tail_ctrl_direction,new_tail_ctrl_radius+tail_radius_adjust));
		setHeadCtrl(head_node.origin().newPoint(head_ctrl_direction,new_head_ctrl_radius+head_radius_adjust));

		// this algorithm can result in the ctrl points being pushed back inside the node. we must prevent that.
		...
		if (Geometric.distance(tail_node.origin(),tailCtrl()) < tail_node.r() + 2*ANCHOR_RADIUS)
		{
			...
			curve.ctrlx1 = tail_node.origin().x + (tail_node.r()+2*ANCHOR_RADIUS)*ctrl_adjust.d.x; 
			curve.ctrly1 = tail_node.origin().y + (tail_node.r()+2*ANCHOR_RADIUS)*ctrl_adjust.d.y;
		}
		if (Geometric.distance(head_node.origin(),headCtrl()) < head_node.r() + 2*ANCHOR_RADIUS + ArrowHead.SHORT_HEAD_LENGTH)
		{
			ctrl_adjust = new Line(head_node.origin(), headCtrl());
			curve.ctrlx2 = head_node.origin().x + (head_node.r()+4*ANCHOR_RADIUS + ArrowHead.SHORT_HEAD_LENGTH)*ctrl_adjust.d.x; 
			curve.ctrly2 = head_node.origin().y + (head_node.r()+4*ANCHOR_RADIUS + ArrowHead.SHORT_HEAD_LENGTH)*ctrl_adjust.d.y;
		}
     ...
     * 
     * @param	rise	The rise factor of the control points from the bisector between node origins
     * @param	angle	The angle start-origin-nodeline and end-origin-nodeline.
     */
	public void calculateCurve(float rise, float angle)
	{
		if (SELECTED_METHOD==1) { calculateCurve1(rise,angle); }
		else
		{
			...
			int padded_radius = head_node.r() + ArrowHead.SHORT_HEAD_LENGTH;
			setTailAnchor(new Point((int)Math.round(tail_node.x() + tail_node.r()*d1.x), (int)Math.round(tail_node.y() + tail_node.r()*d1.y)));
			setHeadAnchor(new Point((int)Math.round(head_node.x() - padded_radius*d2.x), (int)Math.round(head_node.y() - padded_radius*d2.y)));

			// re-create a unit vector from tailAnchor to headAnchor
			d = new UnitVector(tailAnchor(),headAnchor());
			dperp = d.newPerpendicular();			
			
			// create the control points, by moving run in the d direction and rise in the dperp direction from the origin anchors
			float run = Geometric.distance(tailAnchor(),headAnchor())/3;
			setTailCtrl(new Point((int)Math.round(tailAnchor().x + run*d.x - rise*dperp.x), (int)Math.round(tailAnchor().y + run*d.y - rise*dperp.y)));
			setHeadCtrl(new Point((int)Math.round(headAnchor().x - run*d.x - rise*dperp.x), (int)Math.round(headAnchor().y - run*d.y - rise*dperp.y)));
     ...
     * This should only be used for non self loops that are in a custom configuration.
     * This manipulation is not accurate hence re-doing is will not undo it.
     * 
     * @param	reconfigure		Whether or not to adjust the curve and arrowhead points
     */
	public void reverseDirection(boolean reconfigure)
	{
		...
		if (reconfigure)
		{
			// swap the curve points to preserve direction relation to naming convention
			float t = 0;
			t = curve.x1;
			curve.x1 = curve.x2;
			curve.x2 = t;
	...
	public Point selfLoopDirection()
	{
		...
		{ return new Point(self_loop_direction.x,self_loop_direction.y); }
		else
	 ...
	 * 
	 * @param	p	A new value for the tail anchor point.
	 */
	 ...
	 * 
	 * @param	p	A new value for the tail ctrl point.
	 */
	 ...
	 * 
	 * @param	p	A new value for the head ctrl point.
	 */
	 ...
	 * 
	 * @param	p	A new value for the head anchor point.
	 */
	...
	/**
	 * Output a representation of this edge in gml format.
	 * 
	 * @param	out		An initialized PrintWriter for outputing the data to file.
	 */	
	public void printWriterOut(PrintWriter out)
	{
        out.println("        x1 " + curve.x1);
        out.println("        y1 " + curve.y1);
        out.println("        ctrlx1 " + curve.ctrlx1);
        out.println("        ctrly1 " + curve.ctrly1);
        out.println("        ctrlx2 " + curve.ctrlx2);
        out.println("        ctrly2 " + curve.ctrly2);
        out.println("        x2 " + curve.x2);
        out.println("        y2 " + curve.y2);
        if (self_loop_direction != null)
        {
          out.println("        dx " + self_loop_direction.x);
          out.println("        dy " + self_loop_direction.y);
        }
	}	
     ...
     * @param	use_pstricks	Determines export format.
     * @param	draw_dashed		Option to draw as a dashed line.
     */
	public void exportLatex(Box box, LatexPrinter latex_printer, boolean use_pstricks, boolean draw_dashed)
	{
		String dash = "";
		if (use_pstricks)
		{
			if (draw_dashed) { dash = "[linestyle=dashed]"; }
			latex_printer.addCurve("  \\psbezier" + dash + "(" + (curve.x1-box.x1()) + "," + (box.y2()-curve.y1) + ")(" 
			...
			{ 
				...
				d.reverse();
				Point p = new Point(arrowhead.tip(),d,ArrowHead.HEAD_LENGTH/2);
				latex_printer.addArrow("  \\psline[arrowsize=5pt]{->}(" + (p.x-box.x1()) + "," + (box.y2()-p.y) + ")(" + (arrowhead.tip().x-box.x1()) + "," + (box.y2()-arrowhead.tip().y) + ")"); 
			}
			else 
			{ latex_printer.addArrow("  \\psline[arrowsize=5pt]{->}(" + (curve.x2-box.x1()) + "," + (box.y2()-curve.y2) + ")(" + (arrowhead.tip().x-box.x1()) + "," + (box.y2()-arrowhead.tip().y) + ")"); }
		}
		else
		{
			latex_printer.addEscapedCurve("  \\cbezier(" + (curve.x1-box.x1()) + "," + (box.y2()-curve.y1) + ")(" 
														 + (curve.ctrlx1-box.x1()) + "," + (box.y2()-curve.ctrly1) + ")(" 
												         + (curve.ctrlx2-box.x1()) + "," + (box.y2()-curve.ctrly2) + ")(" 
												         + (curve.x2-box.x1()) + "," + (box.y2()-curve.y2) + ")");
			...
			latex_printer.addArrow("  \\put(" + (arrowhead.tip().x-box.x1()) + "," + (box.y2()-arrowhead.tip().y) + "){\\vector(" + Math.round(d.x*100) + "," + Math.round(-d.y*100) + "){0}}");
		}
	}
     ...
     * 
     * @param	t	The parameter of the curve's equation, 0..1 gives placement on curve.
     * @return	A point on this curve.
     */
	public Point calculateBezierPoint(float t)
	{ 
		float x = (1-t) * (1-t) * (1-t) * curve.x1 + 3 * t * (1-t) * (1-t) * curve.ctrlx1 + 3 * t * t * (1-t) * curve.ctrlx2 + t * t * t * curve.x2; 
		float y = (1-t) * (1-t) * (1-t) * curve.y1 + 3 * t * (1-t) * (1-t) * curve.ctrly1 + 3 * t * t * (1-t) * curve.ctrly2 + t * t * t * curve.y2; 
		return new Point(x,y);
	}
	
    /**
     * Checks if a mouse-click landed in any of the regions of this object.
     * If the mouse-click isn't in any of the regions of this object then Edge.R_NONE is returned.
     *
	 * @param	mouse		The current mouse position.
	 * @param	padded		Whether or not we are testing for an exact hit or a nearby hit.
	...
	public int isLocatedSelfLoop(Point mouse, boolean padded)
	{
		...
		int test_radius = ANCHOR_RADIUS;
		if (padded) { test_radius = 2*ANCHOR_RADIUS; }
		if (selfLoopAnchor().isInsideCircle(test_radius,mouse)) { return Edge.R_LOOP; }
		else { return Edge.R_NONE; }
	}
	...
	{
		int test_radius = ANCHOR_RADIUS;
		if (padded) { test_radius = 4*ANCHOR_RADIUS; }
		     if (tailAnchor().isInsideCircle(test_radius,mouse)) { return Edge.R_TAIL_ANCHOR; }
		else if (tailCtrl().isInsideCircle(test_radius,mouse))   { return Edge.R_TAIL_CTRL;   }
		else if (headCtrl().isInsideCircle(test_radius,mouse))   { return Edge.R_HEAD_CTRL;	}
		else if (headAnchor().isInsideCircle(test_radius,mouse)) { return Edge.R_HEAD_ANCHOR; }
		else { return Edge.R_NONE; }
	}
     ...
     * Attempt to automatically layout this curve to a default configuration.
     * 
     * @param	rise_factor	The rise factor of the control points from the bisector between node origins
     * @param	angle		The angle start-origin-nodeline and end-origin-nodeline.
     */
	private void calculateCurve1(float rise_factor, float angle)
	{
		...
		// create the control points, by moving along d1 and d2 away from the centers of the nodes and perpendicular to these directions to achieve rise
		float mag = (float)Math.sqrt(Math.pow(p1.x-p4.x,2) + Math.pow(p1.y-p4.y,2))/3; // one third of the distance between the end points (used to move away from the centers of the nodes)
		float rise = 0; // used to rise from the bisector of the nodes
		if (rise_factor != 0) { rise = mag/rise_factor; } 
		Point p2 = new Point((int)Math.round(tail_node.x() + mag*d.x - rise*dperp.x), (int)Math.round(tail_node.y() + mag*d.y - rise*dperp.y));
		Point p3 = new Point((int)Math.round(head_node.x() - mag*d.x - rise*dperp.x), (int)Math.round(head_node.y() - mag*d.y - rise*dperp.y));
