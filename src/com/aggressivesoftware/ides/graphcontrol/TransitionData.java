...
import com.aggressivesoftware.general.ImageSupport;
...
public class TransitionData 
{
	...
	public TransitionData (GraphingPlatform graphing_platform, Composite parent)
	{
		...
		lbl_symbol.setText(gp.rm.getString("edge.symbol") + ": ");
		...
		lbl_name.setText(gp.rm.getString("edge.name") + ": ");
		...
		lbl_machine_code.setText(gp.rm.getString("edge.machine_code") + ": ");
		...
		lbl_description.setText(gp.rm.getString("edge.description") + ": ");
		...
		lbl_properties.setText(gp.rm.getString("edge.properties") + ": ");
		...
		cbo_controllable.setItems(new String[] {gp.rm.getString("edge.controllable"), gp.rm.getString("edge.uncontrollable")});
		cbo_controllable.select(0);

		cbo_observable = new Combo(cmp_properties, SWT.DROP_DOWN | SWT.READ_ONLY);
		cbo_observable.setItems(new String[] {gp.rm.getString("edge.observable"), gp.rm.getString("edge.unobservable")});
		...
		save_button.setText(gp.rm.getString("edge.save_button"));
		...
		save_as_new_button.setText(gp.rm.getString("edge.save_as_new_button"));
		...
		delete_selected_button.setText(gp.rm.getString("edge.delete_selected_button"));
		...
		lbl_latex.setText(gp.rm.getString("edge.latex") + ": ");
		...
		col_description.setText(gp.rm.getString("edge.description"));
		...
		lbl_trace.setText(gp.rm.getString("edge.trace") + ": ");
	...
	public void draw(Graphics2D g2d)
	{
		...
		{ g2d.drawString(gp.rm.getString("edge.latex_disabled"),5,15); }
		else
		{
			...
			{ g2d.drawString(gp.rm.getString("edge.latex_none"),5,15); }
		}
	}
	...
	public void resize()
	{
		if (gp.tab_folder.getSelectionIndex() == GraphingPlatform.SPECIFICATIONS_TAB)
		{
	...
	public void saveData()
	{
		...
		{
			renderIfNecessary(selected_item, selected_item.getText(SPEC_LATEX), txt_latex.getText());		
	...
	/**
	 * Render the latex code if rendering is requested, enabled, and the string has changed and is not zero length.
	 * 
	 * @param table_item	The TableItem in question.
	 * @param old_value		The old version of the latex code.
	 * @param new_value		The new version of the latex code.
	 */
	private void renderIfNecessary(TableItem table_item, String old_value, String new_value)
	{
		if (gp.sv.use_latex_labels && gp.gc.renderer != null)
		{
			...
			if (new_value.length() > 0)
			{
				...
				{
					// the information has changed so we should re-render
	
					gp.shell.setEnabled(false);
					gp.shell.setCursor(gp.rm.getCursor(gp.rm.WAIT_CURSOR));
					try 
					{ 
						BufferedImage buffered_image = gp.gc.renderer.renderString(new_value);
						table_item.setData("awt", buffered_image); 
						table_item.setData("swt", ImageSupport.safeNewImage(gp.display, table_item.getData("swt"), ConvertSWT.convertToSWT(buffered_image)));
					}
			...
			{
				// code is zero length therefore replace with null.
				table_item.setData("awt",null);
				ImageSupport.safeDispose(table_item.getData("swt"));
				table_item.setData("swt",null);
			}
		}			
		else if (!old_value.equals(new_value))
		{
			// rendering is disabled, and code has changed; therefore, null data.
			table_item.setData("awt",null);
			ImageSupport.safeDispose(table_item.getData("swt"));
			table_item.setData("swt",null);
		}
	...
	/**
	 * Iterate through all TableItems, and render any latex code that has not yet been rendered,
	 * providing that rendering is enabled.  Note that if latex code were changed when rendering
	 * was disabled, it's rendered image would have been set to null.
	 */
	private void renderIfNecessary()
	{
		if (gp.sv.use_latex_labels && gp.gc != null && gp.gc.renderer != null)
		{
			// rendering is requested
			...
			{ 
				if (table_items[i].getText(SPEC_LATEX).length() > 0 && table_items[i].getData("awt") == null)
				{
					// there is code and it isn't rendered
					try 
					{ 
						BufferedImage buffered_image = gp.gc.renderer.renderString(table_items[i].getText(SPEC_LATEX));
						table_items[i].setData("awt", buffered_image); 
						table_items[i].setData("swt", ImageSupport.safeNewImage(gp.display, table_items[i].getData("swt"), ConvertSWT.convertToSWT(buffered_image)));
					}
					catch (Exception e) { e.printStackTrace(); }	
				}
			}	
	...
	public boolean inputData(BufferedReader in, StringTokenizer st, String this_token, String this_line)
	{
		...
		try
		{
			...
			else if (this_token.endsWith("." + spec_strings[SPEC_LATEX]))
			{
				input_item.setText(SPEC_LATEX,Ascii.unEscapeReturn(this_line));
				renderIfNecessary(input_item, "", input_item.getText(SPEC_LATEX));		
				success = true;
			}
	...
	public void fillBlankLabels()
	{
		...
		{ 
			if (gp.sv.use_latex_labels)
			{
				if (table_items[i].getText(SPEC_LATEX).length() < 1)
				{ table_items[i].setText(SPEC_LATEX,table_items[i].getText(SPEC_SYMBOL)); }
			}
			else
			{
