...
public class CubicParamCurve2D extends CubicCurve2D.Float
{
	 ...
	 */
	public CubicParamCurve2D(float x1, float y1, float ctrlx1, float ctrly1,
	 ...
	 * 
	 * @param curve
	 *            the curve whose control points to copy
	 */
	 ...
	 *            to 1)
	 */
	public void subdivide(CubicParamCurve2D part1, CubicParamCurve2D part2,
			float t)
	{
		float t2 = t * t;
		float t3 = t2 * t;
	...
	public Point2D.Float getPointAt(float t)
	{
		...
		return new Point2D.Float((float)left.getP2().getX(), (float)left
				.getP2().getY());
	}
	...
	{
		return 2 * (float)(Math.sqrt((x1 - ctrlx1) * (x1 - ctrlx1)
				+ (y1 - ctrly1) * (y1 - ctrly1))
				+ Math.sqrt((ctrlx2 - ctrlx1) * (ctrlx2 - ctrlx1)
						+ (ctrly2 - ctrly1) * (ctrly2 - ctrly1)) + Math
				.sqrt((x2 - ctrlx2) * (x2 - ctrlx2) + (y2 - ctrly2)
						* (y2 - ctrly2)));
	}
	...
	/**
	 * Returns a string representing the control points of this curve. Formatted
	 * by square brackets with each point comma-delimited.
	 * 
	...
	{
		return "[" + getP1() + ", " + getCtrlP1() + ", " + getCtrlP2() + ", "
				+ getP2() + "]";
	}
}
