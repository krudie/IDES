 ...
 */
public class SupEventSetThumbnail extends Box implements Presentation,
		SupEventSetSubscriber
{
	...
	/**
	 * Generates the text to display in the thumbnail. The first line
	 * (numEvents) shows the number of events. The second line (space) is blank
	 * for readability. The third line (eventNamesLine1) is the first few events
	 * (in alphabetical order), if they exist (up to 15 characters). The fourth
	 * line (eventNamesLine2) is the next few events (in alphabetical order), if
	 * they exist (up to 15 characters). The fifth line (etc) is simply "..."
	 * which is displayed if there are more events that are not displayed on
	 * EventNamesLine1 or EventNameLine2.
	 */
	public void refresh()
	{
		if (model.size() == 1)
		...
		for (Iterator<SupervisoryEvent> i = model.iteratorSupervisory(); i
				.hasNext();)
		{
			events.add(i.next());
		}
