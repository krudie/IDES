...
public class GraphLink implements PresentationElement
{
	...
	public void draw(Graphics g)
	{
		...
		// String text=""+link.getEventLeft().getSymbol()+" =
		// "+link.getEventRight().getSymbol();
	...
	public Float getLocation()
	{
		...
		return new Point2D.Float(
				Math.min(leftBlock.getLocation().x, rightBlock.getLocation().x)
						+ Math.abs(leftBlock.getLocation().x
								- rightBlock.getLocation().x) / 2,
				(Math.abs(leftBlock.getLocation().x
						- rightBlock.getLocation().x) / 2)
						* Geometry.slope(leftBlock.getLocation(), rightBlock
								.getLocation())
						+ (Math.min(leftBlock.getLocation().x, rightBlock
								.getLocation().x) == leftBlock.getLocation().x ? leftBlock
								.getLocation().y
