...
public class SupervisorSelectDialog extends EscapeDialog
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8764041742648428797L;
		...
		okButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				if (modelList.getSelectedIndices().length != needToSelect)
				{
					...
					Hub.displayAlert("Please select " + needToSelect
							+ " supervisors.");
				}
				else
				{
	...
	{
		needToSelect = count;
		Collection<FSAModel> models = Hub
		...
		{
			ret.add((FSASupervisor)Hub
					.getWorkspace().getModel((String)selected[i]));
		}
		return ret;
	}
