...
public class SelectionTool extends DrawingTool {
	...
	public SelectionTool()
	{
		...
		cursor = toolkit.createCustomCursor(toolkit.createImage(Hub.getResource("images/cursors/modify_.gif")), new Point(0,0), "SELECT");		
	}
	...
	public void handleMouseDragged(MouseEvent m)
	{
		...
		updateDragDisplay(
				new Rectangle(Math.min(origin.x, m.getX()),
				Math.min(origin.y, m.getY()),
				Math.abs(m.getX()-origin.x),Math.abs(m.getY()-origin.y)));		
		context.repaint();
	}
	...
	public void handleMousePressed(MouseEvent m)	
	{
		...
		if(!intersectsSelection(m.getPoint()))
		{
			...
			dragSelection=false;
			for(GraphBlock b:((TemplateGraph)context.getLayoutShell()).getBlocks())
			{
	...
	public void handleMouseClicked(MouseEvent m)
	{
		...
		{
			Point2D.Float mp=context.localToScreen(new Point2D.Float(m.getX(),m.getY()));
			...
			if(selectedB!=null)
			{
				...
				delItem.addActionListener(new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
						if(delBlock!=null)
						{
							((TemplateGraph)context.getLayoutShell()).remove(delBlock);
							delBlock=null;
							promptOn=false;
						}
					}
			...
			else if(selectedL!=null)
			{
				...
				delItem.addActionListener(new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
						if(delLink!=null)
						{
							((TemplateGraph)context.getLayoutShell()).remove(delLink);
							delLink=null;
							promptOn=false;
						}
					}
		...
		else if(m.getButton()==MouseEvent.BUTTON1&&m.getClickCount()==2)
		{
			...
			if(selected!=null)
			{
				...
				BlockLabellingDialog.showAndLabel(context,
						(TemplateGraph)context.getLayoutShell(),selected,
						this);
			}
		}
	}
