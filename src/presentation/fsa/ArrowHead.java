...
public class ArrowHead extends Polygon
{
	...
	/**
	 * Construct a default arrowhead at location (0,0) and orientation in
	 * direction (0,1) ie. pointing down.
	 */
	public ArrowHead()
	{
		reset();
	...
	{
		setLocationAndDirection(new Point2D.Float(
				(float)base.getX(),
				(float)base.getY()), new Point2D.Float(
				(float)dir.getX(),
				(float)dir.getY()));
	}
	 ...
	 * orient with direction vector <code>dir</code>. NOTE this method is not
	 * used when drawing the arrowhead on edges since problems occurred with
	 * flicker. When drawing arrowhead on canvas use AffineTransform on the
	 * Graphics context. FIXME Since the default orientation is stored in memory
	 * but the arrowhead is only rotated when rendered, checking for
	 * intersections will not work properly.
	 * 
	 * @param base
	...
	/**
	 * Sets arrowhead to default location (0,0) and orientation in direction
