...
public class EdgePopup extends JPopupMenu
{
	...
	private JMenuItem miEditEvents, miStraighten, miSymmetrize, miArcMore,
			miArcLess;
	...
	protected EdgePopup(GraphDrawingView gdv, Edge edge)
	{
		...
		{
			miStraighten = new JMenuItem(new UIActions.StraightenEdgeAction(
					gdv.graphModel,
					edge));
			// miStraighten.addActionListener(listener);
			add(miStraighten);

			miSymmetrize = new JMenuItem(new UIActions.SymmetrizeEdgeAction(
					gdv.graphModel,
					edge));
			add(miSymmetrize);

			miArcMore = new JMenuItem(new UIActions.ArcMoreEdgeAction(
					gdv.graphModel,
					edge));
			// miArcMore.addActionListener(listener);
			add(miArcMore);

			miArcLess = new JMenuItem(new UIActions.ArcLessEdgeAction(
					gdv.graphModel,
					edge));
