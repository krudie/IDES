...
public class EdgeLabellingDialog extends EscapeDialog
{
	...
	private EdgeLabellingDialog(Frame owner, EventsModel eventsModel)
	{
		...
		DocumentListener al = new DocumentListener()
		{
			...
			private void configStuff(String symbol)
			{
				if (!listAvailableEvents.existsElement(symbol)
						&& !listAssignedEvents.existsElement(symbol))
				{
					createBox
		...
		createBox.setBorder(BorderFactory.createTitledBorder(BorderFactory
				.createLineBorder(UIManager.getColor("TextField.shadow"), 1),
				Hub.string("enterAssignEvent")));
					...
					public void valueChanged(ListSelectionEvent e)
					{
						...
						if (!lsm.isSelectionEmpty())
						{
							...
							if (o != null)
							{
								...
								{
									textField
											.setText(selectedEvent.getSymbol());
								}
								if (inserted
												...
												.getText().length())
								{
									...
									textField.setSelectionEnd(textField
											.getText().length());
									textField.setSelectionStart(originalText
											.length());
								}
		...
		buttonOK.setPreferredSize(new Dimension(Math.max(buttonOK.getWidth(),
				buttonCancel.getWidth()), buttonOK.getHeight()));
		buttonOK.invalidate();
		buttonCancel
				.setPreferredSize(new Dimension(Math.max(buttonOK.getWidth(),
						buttonCancel.getWidth()), buttonCancel.getHeight()));
	...
	private class MutableList extends JList
	{
		...
		private int findInsertionPoint(Comparable entry)
		{
			...
			{
				int c = ((Comparable)getContents().elementAt(i))
						.compareTo(entry);
				if (c >= 0)
				{
					return i;
