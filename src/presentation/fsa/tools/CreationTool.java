...
public class CreationTool extends DrawingTool
{
	...
	public void handleMouseClicked(MouseEvent me)
	{
		...
		if (drawingSelfLoop())
		{
			...
			{ // double click should activate text (labelling) tool
				abortEdge();
	...
	public void handleMousePressed(MouseEvent me)
	{
		...
		GraphElement selectedElement = ContextAdaptorHack.context
				.getSelectedElement();
		if (selectedElement instanceof CircleNode
				|| ContextAdaptorHack.context.getSelectedElement() == null)
		{ // If a node or an empty space is clicked
			if (ContextAdaptorHack.context.getAvoidNextDraw() == true)
			{
				return;
			}
			startNode = (CircleNode)ContextAdaptorHack.context
					.getSelectedElement();
			if (!drawingEdge)
			{
				if (startNode != null)
	...
	public void handleMouseReleased(MouseEvent me)
	{
		...
		ContextAdaptorHack.context.updateCurrentSelection(me.getPoint());
		ContextAdaptorHack.context.repaint();
		GraphElement selection = ContextAdaptorHack.context
				.getSelectedElement();
		try
		{
		...
		if (startNode == endNode && endNode == sourceNode && drawingEdge
				&& !dragging && firstClick)
		{ // drawing edge by not dragging
			// IDEA To fix conflict with TextTool, delay creation of self loops
			// until we know if user has double clicked.
		...
		else if (startNode != null && startNode == endNode
				&& startNode == sourceNode && dragging)
		{ // select source node, keep drawing edge by mouse move (not
			// dragging)
			dragging = false;
			firstClick = true;
			finishEdge();
		...
		else if (startNode == endNode && startNode != sourceNode
				&& endNode != null)
		{ // select target node, finish drawing edge by mouse move
			finishEdge();
			firstClick = false;
		}
		...
		else if (drawingEdge && dragging && endNode != null)
		{ // Assumption: sourceNode != null
			finishEdge();
		}
		else
		{
	...
	{
		return startNode == endNode && endNode == sourceNode && drawingEdge
				&& !dragging;
	}
	...
	private void finishSelfLoop()
	{
		...
		// cmd.execute();
		new GraphActions.CreateEdgeAction(ContextAdaptorHack.context
				.getGraphModel(), targetNode, targetNode).execute();
		sourceNode = null;
		targetNode = null;
		ContextAdaptorHack.context.clearCurrentSelection();
	}
	...
	private void createNode(Point point)
	{
		...
		// cmd.execute();
		new GraphActions.CreateNodeAction(ContextAdaptorHack.context
				.getGraphModel(), new Point2D.Float(point.x, point.y))
				.execute();
		abortEdge();
		// dragging = false;
		sourceNode = null;
	...
	private void finishEdgeAndCreateTarget(Point point)
	{
		...
		new GraphActions.CreateNodeAction(
				allEdits,
				ContextAdaptorHack.context.getGraphModel(),
				new Point2D.Float(point.x, point.y),
				nodeBuffer).execute();
		new GraphActions.CreateEdgeAction(allEdits, ContextAdaptorHack.context
				.getGraphModel(), edge.getSourceNode(), nodeBuffer[0])
				.execute();
	...
	private BezierEdge beginEdge(CircleNode n1)
	{
		...
		layout.computeCurve((CircleNodeLayout)n1.getLayout(), n1
	 ...
	 * point. If the focused point is a point of the source node, the layout
	 * will capture the possible intencion of a reflexive edge and IDES will
	 * draw a reflexive layout.
	 * 
	 * @param e
	...
	private void finishEdge()
	{
		...
		if (edge != null)
		{
			...
			// cmd.execute();
			new GraphActions.CreateEdgeAction(ContextAdaptorHack.context
					.getGraphModel(), edge.getSourceNode(), targetNode)
					.execute();
	...
	public void handleMouseDragged(MouseEvent me)
	{
		...
		if (dragging && drawingEdge
				|| (sourceNode == targetNode && targetNode != null))
		{
