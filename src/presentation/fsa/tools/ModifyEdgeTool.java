...
public class ModifyEdgeTool extends DrawingTool
{
	...
	public void handleMousePressed(MouseEvent m)
	{
		...
		if (ContextAdaptorHack.context.hasCurrentSelection())
		{
			...
			if (temp != null)
			{
				...
				{
					previousDirection = (Point2D.Float)((InitialArrow)edge)
							.getDirection().clone();
		...
		if (ContextAdaptorHack.context.hasCurrentSelection())
		{
			...
			else
			{
				...
				ContextAdaptorHack.context
						.getCurrentTool().handleMousePressed(m);
			}
		}
		else
		{
	...
	{
		ContextAdaptorHack.context.setTool(ContextAdaptorHack.context
				.getPreferredTool());
		dragging = false;
		edge = null;
	}
	...
	{
		if (edge.getHandler().isVisible()
				&& edge.getHandler().intersects(point))
		{
	...
	public void handleMouseDragged(MouseEvent m)
	{
		...
		if (dragging)
		{
			...
			edge.setPoint(new Point2D.Float(m.getPoint().x, m.getPoint().y),
					pointType);
			ContextAdaptorHack.context.repaint();
		}
	}
	...
	public void handleMouseReleased(MouseEvent m)
	{
		...
		if (dragging)
		{ // TODO check to see if edge has been changed
			...
			{
				new NodeActions.ModifyInitialArrowAction(
						ContextAdaptorHack.context.getGraphModel(),
						(InitialArrow)edge,
						previousDirection).execute();
			}
			else
			{
				new EdgeActions.ModifyAction(ContextAdaptorHack.context
						.getGraphModel(), edge, previousLayout).execute();
			}
			ContextAdaptorHack.context.repaint();
			dragging = false;
			switchTool();
