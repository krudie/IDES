...
public class SelectionTool extends DrawingTool
{
	...
	public void handleMouseDragged(MouseEvent me)
	{
		...
		if (!endPoint.equals(startPoint))
		{
			...
			topLeftPt.setLocation(Math.min(startPoint.x, endPoint.x), Math
					.min(startPoint.y, endPoint.y));
			d.setSize(Math.abs(endPoint.x - startPoint.x), Math.abs(endPoint.y
					- startPoint.y));
			ContextAdaptorHack.context
					.getSelectionArea().setLocation(topLeftPt);
			ContextAdaptorHack.context.getSelectionArea().setSize(d);
			ContextAdaptorHack.context
					.updateCurrentSelection(ContextAdaptorHack.context
							.getSelectionArea());
	...
	public void handleMousePressed(MouseEvent me)
	{
		...
		if (ContextAdaptorHack.context.getSelectedGroup().size() > 1
				&& ContextAdaptorHack.context.getSelectedGroup().intersects(me
						.getPoint()))
		{
			ContextAdaptorHack.context.highlightCurrentSelection(true);
		...
		if (ContextAdaptorHack.context.hasCurrentSelection()
				&& ContextAdaptorHack.context.getSelectedElement() instanceof Edge)
		{ // KLUGE instanceof is evidence of poor design
