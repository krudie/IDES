...
public class CircleNode extends Node
{
	...
	/**
	 * Refreshes the visual representation of the node from the underlying state
	 * and layout data. Also recomputes the layout for adjacent edges should
	 * they be affected by changes to the node's size or position.
	 */
	...
	public void refresh()
	{
		...
		float radius = (float)Math.max(labelBounds.getWidth() / 2 + 2
				* CircleNodeLayout.RADIUS_MARGIN,
				CircleNodeLayout.DEFAULT_RADIUS + 2
						* CircleNodeLayout.RADIUS_MARGIN);
	...
	/**
	 * @return bounding rectangle for union of this node with all of its
	 *         children.
	 */
	@Override
	...
	public boolean intersects(Point2D p)
	{
		...
		if (state.isInitial())
		{
			return circle.intersects(p.getX() - 5, p.getY() - 5, 10, 10)
					|| initialArrow.intersects(p); // (p.getX() - 4, p.getY() -
			// 4, 8, 8);
		}
		return circle.intersects(p.getX() - 5, p.getY() - 5, 10, 10);
	}
	...
	/**
	 * NOTE since we are sharing references to unique node objects, this
	 * shouldn't be necessary. But just in case...
	 * 
	 * @param n
	 *            an instance of a CircleNode
	 ...
	 * 
	 * @param dx
	 *            the displacement in the horizontal dimension
	 * @param dy
	 *            the displacement in the vertical dimension
	 */
	@Override
	...
	protected Rectangle getSquareBounds()
	{
		...
		return new Rectangle(BentoBox
				.convertFloatToInt(nodeLocation.x - radius), BentoBox
				.convertFloatToInt(nodeLocation.y - radius), BentoBox
				.convertFloatToInt(radius * 2), BentoBox
				.convertFloatToInt(radius * 2));
	}
	 ...
	 * 
	 * @param layout
	 *            the graphical layout data to be set
	 */
