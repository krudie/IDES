...
{
	/**
	 * Old size of the correspondent edge. When this variable changes the value,
	 * the refresh method call routines to recompute the curve paramethers.
	 */
	 ...
	 * @param node
	 * @param t
	 *            a transition this represented by this edge
	 */
	 ...
	 * If not enough space, places this edge in the default position. TODO If
	 * not enough space, looks for a layout that doesn't clobber another
	 * reflexive edge.
	 */
	@Override
	...
	public String createExportString(Rectangle selectionBox, int exportType)
	{
		...
		if (exportType == GraphExporter.INT_EXPORT_TYPE_PSTRICKS)
		{
			...
			// Now for the label
			if ((getBezierLayout().getText() != null)
	...
	{
		return ((ReflexiveLayout)getLayout())
				.getCurve().getBounds().union(getLabel().bounds());
	}
	...
	public class ReflexiveHandler extends EdgeHandler
	{
		...
		/**
		 * Refreshes the position of this handler based on location of midpoint
		 * of edge.
		 */
		@Override
		public void refresh()
		{
			...
			midpoint = Geometry.translate(midpoint, endpoint.getX(), endpoint
					.getY());
			anchor = new Ellipse2D.Double(midpoint.getX() - RADIUS, midpoint
					.getY()
					- RADIUS, d, d);
			// anchor = new
		 ...
		 * 
		 * @param g
		 *            the graphics context in which to render this handler
		 */
		@Override
