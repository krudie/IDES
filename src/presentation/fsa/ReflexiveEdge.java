...
//		insertAmong(n);
//		}
		((ReflexiveLayout)getLayout()).setAxis((Point2D)((ReflexiveLayout)getLayout()).computeBestDirection(this.getSourceNode()));
		computeEdge();
	}
	/**
	...
	public void insertAmong(Set<Edge> neighbours) {	
		...
		while(BezierEdgePlacer.tooClose(this, neighbours) && alpha < 360) {
			...
			setMidpoint(Geometry.add(getSourceNode().getLocation(), ((ReflexiveLayout)getLayout()).getAxis()));
			computeEdge();
			alpha ++;
		}
