...
public class BezierEdge extends Edge
{
	...
	/**
	 * Creates a new directed Bezier edge emanating from <code>source</code>,
	 * with null target node, no transition, and having the given layout data.
	 ...
	 *            the target node
	 * @param t
	 *            the transition to represent
	 */
	public BezierEdge(BezierLayout layout, Node source, Node target,
			FSATransition t)
	{
	...
	public void draw(Graphics g)
	{
		...
		if (!hasUncontrollableEvent()
				&& getBezierLayout().getControllableMarker() != null)
		{
		...
		if (tEndPt != null)
		{
			basePt = Geometry.add(tEndPt, Geometry.scale(unitArrowDir,
					-(ArrowHead.SHORT_HEAD_LENGTH + 2)));
		}
		else
		{
			basePt = Geometry.add(getBezierLayout().getCurve().getP2(),
					Geometry.scale(unitArrowDir,
							-(ArrowHead.SHORT_HEAD_LENGTH + 2)));
		}
	...
	public void refresh()
	{
		...
		float sourceT = intersectionWithBoundary(getSourceNode().getShape(),
				sourceEndPt,
				SOURCE_NODE);
		...
		if (getTargetNode() != null)
		{
			...
			float tTarget = intersectionWithBoundary(getTargetNode().getShape(),
					targetEndPt,
					TARGET_NODE);
		...
		Point2D.Float location = Geometry.add(new Point2D.Float((float)midpoint
				.getX(), (float)midpoint.getY()), getBezierLayout()
				.getLabelOffset());
	 ...
	 * Computes and returns a unit vector indicating the direction in which the
	 * arrowhead should point.
	 * 
	...
	private Point2D.Float computeArrowDirection()
	{
		...
		if (target == null)
		{
			...
			if (bL != null)
			{
				...
				if (curve != null)
				{
					return Geometry.unit(Geometry.subtract(curve.getP2(), curve
							.getCtrlP2()));
				}
			}
			return null;
	...
	/**
	 * Returns whether p intersects with this edge. FIXME change to return true
	 * iff intersects VISIBLE part of curve.
	 * 
	...
	public boolean intersects(Point2D p)
	{
		...
		if (isSelected() && getHandler().isVisible())
		{
			...
			return hit
					...
					|| getLabel().intersects(p) || getHandler().intersects(p);
		}
		else
		{
			// expand the intersection point to an 8 by 8 rectangle
	...
	public Point2D.Float getP1()
	{
		return new Point2D.Float(
				(float)getBezierLayout().getCurve().getX1(),
	...
	{
		return new Point2D.Float(
				(float)getBezierLayout().getCurve().getX2(),
	...
	{
		return new Point2D.Float((float)getBezierLayout()
				.getVisibleCurve().getCtrlX1(), (float)getBezierLayout()
				.getVisibleCurve().getCtrlY1());
	}
	...
	{
		return new Point2D.Float((float)getBezierLayout()
				.getCurve().getCtrlX1(), (float)getBezierLayout()
				.getCurve().getCtrlY1());
	}
	...
	{
		return new Point2D.Float((float)getBezierLayout()
				.getVisibleCurve().getCtrlX2(), (float)getBezierLayout()
				.getVisibleCurve().getCtrlY2());
	}
	...
	{
		return new Point2D.Float((float)getBezierLayout()
				.getCurve().getCtrlX2(), (float)getBezierLayout()
				.getCurve().getCtrlY2());
	}
	 ...
	 * 
	 * @param layout
	 *            graphical layout data to be set
	 */
	@Override
	...
	public void translate(float x, float y)
	{
		...
		if (l.isRigidTranslation())
		{
			...
			// displacement.
			curve.setCurve(curve.getX1() + x, curve.getY1() + y, curve
					.getCtrlX1()
					+ x, curve.getCtrlY1() + y, curve.getCtrlX2() + x, curve
					.getCtrlY2()
					+ y, curve.getX2(), curve.getY2() + y);
	...
	public String createExportString(Rectangle selectionBox, int exportType)
	{
		...
		if (!(selectionBox.contains(edgeP1) && selectionBox.contains(edgeP2)
				&& selectionBox.contains(edgeCTRL1) && selectionBox
				.contains(edgeCTRL2)))
		{
		...
		if (exportType == GraphExporter.INT_EXPORT_TYPE_PSTRICKS)
		{
			...
			else
			{
				...
				exportString += "]{->}"
						...
						+ ","
						+ (selectionBox.y + selectionBox.height - edgeCTRL1
								.getY())
						+ ")("
						+ (edgeCTRL2.getX() - selectionBox.x)
						+ ","
						+ (selectionBox.y + selectionBox.height - edgeCTRL2
								.getY())
						+ ")("
			...
			if (!hasUncontrollableEvent())
			{
				...
				if (controlLine != null)
				{
					exportString += "\\psline("
							...
							+ ","
							+ (selectionBox.y + selectionBox.height - controlLine
									.getY1())
							+ ")("
			...
			// Now for the label
			if ((edgeLayout.getText() != null)
					&& (getLabel().getText().length() > 0))
			{
				exportString += "  "
						+ getLabel().createExportString(selectionBox,
								exportType);
			}
		}
	...
	public Rectangle bounds()
	{
		...
		float minX = BentoBox.getMinValue(edgeP1.x,
				edgeP2.x,
				edgeCTRL1.x,
				edgeCTRL2.x);
		float minY = BentoBox.getMinValue(edgeP1.y,
				edgeP2.y,
				edgeCTRL1.y,
				edgeCTRL2.y);
		float maxX = BentoBox.getMaxValue(edgeP1.x,
				edgeP2.x,
				edgeCTRL1.x,
				edgeCTRL2.x);
		float maxY = BentoBox.getMaxValue(edgeP1.y,
				edgeP2.y,
				edgeCTRL1.y,
				edgeCTRL2.y);

		return new Rectangle(BentoBox.convertFloatToInt(minX), BentoBox
				.convertFloatToInt(minY), BentoBox.convertFloatToInt(maxX
				- minX), BentoBox.convertFloatToInt(maxY - minY));
	}
	 ...
	 *            the point to set
	 * @param pointType
	 *            see <code>P1, CTRL1, CTRL2, P2</code>
	 */
	@Override
	 ...
	 * 
	 * @param symbol
	 *            the event symbol to be displayed
	 */
	@Override
	 ...
	 * @param nL2
	 *            the layout data for the target node
	 */
	...
	public void computeEdge()
	{
		...
		{
			getBezierLayout().computeCurve((CircleNodeLayout)getSourceNode()
					.getLayout(),
					(CircleNodeLayout)getTargetNode().getLayout());
			refresh();
		}
	...
	/**
	 * A simple binary search starting within the boundary of the given node
	 * shape. Sets the coordinates of <code>intersection</code> to the point
	 * where this curve intersects the boundary of <code>nodeShape</code>. If
	 * endpoints are both inside node (self-loop or overlapping target and
	 * source), then returns 0.5. Precondition: node is not null and
	 * intersection is not null
	 * 
	 * @param nodeShape
	 *            the shape representing this node
	 * @param type
	 *            ignored
	 * @param intersection
	 *            acts as a second return value; to be set to the point of
	...
	protected float intersectionWithBoundary(Shape nodeShape,
			Point2D.Float intersection, int type)
	{
		...
		// and source)
		// FIXME
		if (nodeShape.contains(curve.getP1())
				&& nodeShape.contains(curve.getP2()))
		{
			return 0.5f;
		}
	 ...
	 * its source and target nodes. Other edges' layouts are adjusted as
	 * necessary.
	 * 
	 * @param neighbours
	 *            the set of edges between the source and target nodes
	 */
