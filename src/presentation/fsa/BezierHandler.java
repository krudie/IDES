...
public class BezierHandler extends EdgeHandler
{
	...
	public void refresh()
	{
		...
		anchors[BezierLayout.P1] = new Ellipse2D.Double(getEdge().getP1().x
				- RADIUS / 2, getEdge().getP1().y - RADIUS / 2, RADIUS, RADIUS);
		anchors[BezierLayout.CTRL1] = new Ellipse2D.Double(
				getEdge().getCTRL1().x - RADIUS,
				getEdge().getCTRL1().y - RADIUS,
				d,
				d);
		anchors[BezierLayout.CTRL2] = new Ellipse2D.Double(
				getEdge().getCTRL2().x - RADIUS,
				getEdge().getCTRL2().y - RADIUS,
				d,
				d);
		anchors[BezierLayout.P2] = new Ellipse2D.Double(getEdge().getP2().x
				- RADIUS / 2, getEdge().getP2().y - RADIUS / 2, RADIUS, RADIUS);
	...
	public void draw(Graphics g)
	{
		...
		if (visible)
		{
			...
			for (int i = 1; i < 3; i++)
			{ // don't display end point circles since not moveable.
				g2d.draw(anchors[i]);
			}
	...
	/**
	 * Returns true if p intersects one of the control point circles and stores
	 * the index of the last intersected point; if false the next call to
