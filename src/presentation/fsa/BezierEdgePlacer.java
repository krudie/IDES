...
public class BezierEdgePlacer
{
	...
	/**
	 * Inserts <code>edge</code> among the given set of other edges, where all
	 * edges connect the same pair of nodes. Computes the layouts for each edge
	 * such that they are distributed comfortably along the circumferences of
	 * each end node. TODO change this algorithm so that it accommodates
	 * reflexive edges and initial arrows (adjacent on only one of
	 * <code>edge</code>'s end nodes).
	 * 
	 * @param edge
	 *            the edge to be inserted
	 * @param otherEdges
	 *            set of all other edges connecting the same pair of nodes
	 */
	 ...
	 */
	private static BezierLayout setToOutermostFreeLayout(Edge edge,
			Set<Edge> otherEdges)
	{
		...
		if (layout != null)
		{
			...
			{
				double maxAngle = (Math.abs(layout.angle1) > Math
						.abs(layout.angle2) ? layout.angle1 : layout.angle2);
	 ...
	 * 
	 * @see CubicCurve2D.flatness() Precondition: <code>edges</code> is not
	 *      empty
	 * @param edges
	 *            the set of edges
	...
	private static BezierLayout findOutermostTakenPosition(Set<Edge> edges)
	{
		...
		for (Edge edge : edges)
		{
			double flatness = ((BezierLayout)edge.getLayout())
					.getCurve().getFlatness();
			if (flatness >= max)
			{
				max = flatness;
	 ...
	 * @param edges
	 *            the set of edges
	 * @return true iff the one or more endpoints of <code>edge1</code> is
	...
	{
		/*
		 * TODO find a nice-looking minimum comfortable distance between
		 * endpoints to allow margins for arrow head along node boundary. For
		 * now just use:
		 */
		...
		for (Edge edge : edges)
		{
			...
			if (!edge.equals(edge1)
					&& ((edge.getSourceEndPoint().distance(edge1
							.getSourceEndPoint()) < min)
							|| (edge.getSourceEndPoint().distance(edge1
									.getTargetEndPoint()) < min)
							|| (edge.getTargetEndPoint().distance(edge1
									.getTargetEndPoint()) < min) || (edge
							.getTargetEndPoint().distance(edge1
									.getSourceEndPoint()) < min)))
			{
				return true;
			}
