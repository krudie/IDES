...
public class BezierLayout extends GraphicalLayout implements Serializable
{
	 ...
	 * storage since this information should only be stored in the label's
	 * layout
	 */
	 ...
	 * 
	 * @param bezierControls
	 *            the control points for the edge
	 */
	...
	/**
	 * Creates a layout for a bezier edge with the given control points and
	 * event names.
	 * 
	 * @param bezierControls
	 *            the control points for the edge
	...
	/**
	 * Constructs an edge layout object for a straight, directed edge connecting
	 * the nodes with source and target layouts <code>n1</code> and
	 * <code>n2</code> respectively.
	 * 
	 * @param sourceLayout
	 *            layout for source node
	 * @param targetLayout
	 *            layout for target node
	 */
	public BezierLayout(CircleNodeLayout sourceLayout,
			CircleNodeLayout targetLayout)
	{
		curve = new CubicParamCurve2D();
	...
	/**
	 * Creates a layout with same edge and curve as <code>other</code> and
	 ...
	 * 
	 * @param s
	 *            layout for source node, s != null
	 * @param t
	 *            layout for target node, t != null
	 */
	public void computeCurve(CircleNodeLayout s, CircleNodeLayout t)
	{
		...
		if (s.equals(t))
		{
			...
			ctrls[P1] = Geometry.add(centre1, Geometry.rotate(Geometry
					.scale(UNIT_VERTICAL, targetRadius), angle1));
			ctrls[P2] = Geometry.add(centre1, Geometry.rotate(Geometry
					.scale(UNIT_VERTICAL, targetRadius), angle2));
			ctrls[CTRL1] = Geometry.add(ctrls[P1], Geometry.rotate(Geometry
					.scale(UNIT_VERTICAL, targetRadius), angle1));
			ctrls[CTRL2] = Geometry.add(ctrls[P2], Geometry.rotate(Geometry
					.scale(UNIT_VERTICAL, targetRadius), angle2));
		}
		else
		{
			...
			if (isStraight())
			{ // compute a default straight edge
				angle1 = 0;
				angle2 = 0;
				s1 = DEFAULT_CONTROL_HANDLE_SCALAR;
				s2 = DEFAULT_CONTROL_HANDLE_SCALAR;

				ctrls[CTRL1] = Geometry.add(ctrls[P1], Geometry.scale(unitBase,
						(float)(norm * s1)));
				ctrls[CTRL2] = Geometry.add(ctrls[P2], Geometry.scale(unitBase,
						-(float)(norm * s2)));

			}
			else
			{ // recompute the edge preserving the shape of the curve

				ctrls[CTRL1] = Geometry.add(ctrls[P1], Geometry.rotate(Geometry
						.scale(base, (float)s1), angle1));
				ctrls[CTRL2] = Geometry.add(ctrls[P2], Geometry.rotate(Geometry
						.scale(base, -(float)s2), angle2));
	 ...
	 * <code>s</code>, the layout for the source node to endpoint
	 * <code>c2</code>.
	 * 
	 * @param s
	 *            layout for source node
	 * @param endPoint
	 *            endpoint for the edge
	 */
	public void computeCurve(CircleNodeLayout s, Point2D.Float endPoint)
	{
		...
		else
		{
			...
			ctrls[CTRL1] = Geometry.add(ctrls[P1], Geometry.scale(unit,
					(float)(norm * s1)));
			ctrls[CTRL2] = Geometry.add(ctrls[P2], Geometry.scale(unit,
					(float)(-norm * s2)));
		}
		curve.setCurve(ctrls, 0);
		Point2D midpoint = Geometry.midpoint(curve);
	...
	/**
	 * Computes and store the shape of the curve so that it can be preserved
	 * under compression or expansion due to movement of its end nodes. Computes
	 * and stores: s1 scalar |(CTRL1 - P1)|/|(P2-P1)| s2 scalar |(CTRL2 -
	 * P2)|/|(P1-P2)| angle1 angle between (CTRL1 - P1) and (P2-P1) angle2 angle
	 * between (CTRL2 - P2) and (P1-P2) In case of self-loop stores angles from
	 * UNIT_VERTICAL to (CTRL2 - P2) and (CTRL1 - P1) and scalars are simply the
	 * lengths of (CTRL2 - P2) and (CTRL1 - P1).
	 */
	protected void updateAnglesAndScalars()
	{
		...
		Point2D.Float p1c1 = Geometry
				.subtract(curve.getCtrlP1(), curve.getP1());
		Point2D.Float p2c2 = Geometry
				.subtract(curve.getCtrlP2(), curve.getP2());
	 ...
	 * Precondition: <code>index</code> is a valid index i.e. P1, P2, CTRL1,
	 * CTRL2. Precondition: point != null
	 * 
	 * @param point
	 *            the value of the control point
	 * @param index
	 *            the index of the control point to be set
	 */
	...
	/**
	 * Returns the visible portion of the curve i.e. the part that is outside
	 * the boundaries of both source and target nodes; null if no such segment
	 * exists.
	 * 
	 * @return the portion of the curve that is external to both source and
	 *         target nodes, null if no such segment exists
	 */
	...
	public Line2D getControllableMarker()
	{
		...
		Point2D.Float vector = Geometry.unit(new Point2D.Float(part.ctrlx2
				- part.x2, part.ctrly2 - part.y2));
		vector = Geometry.rotate(vector, Math.PI / 2);
		vector = Geometry.scale(vector, 5);
		return new Line2D.Float(part.x2 + vector.x, part.y2 + vector.y, part.x2
				- vector.x, part.y2 - vector.y);
	}

	/**
	 * Increase the arc on this edge layout. Increases the angle of the tangents
	 * to the curve, clockwise around circumference of source node.
	 */
	...
	/**
	 * Increase the arc on this edge layout. Increases the angle of the tangents
	 * to the curve, clockwise around circumference of source node if
	 ...
	 * and tangent length by 20%. If angle < DEFAULT_CONTROL_HANDLE_ANGLE / 2,
	 * set it to 0 (i.e. flatten the curve).
	 */
	protected void arcLess()
	{
		if (Math.abs(angle1) < DEFAULT_CONTROL_HANDLE_ANGLE / 2)
		{ // EPSILON){
			angle1 = 0;
			s1 = DEFAULT_CONTROL_HANDLE_SCALAR;
		}
		...
		if (Math.abs(angle2) < DEFAULT_CONTROL_HANDLE_ANGLE / 2)
		{ // EPSILON){
			angle2 = 0;
			s2 = DEFAULT_CONTROL_HANDLE_SCALAR;
		}
	...
	/**
	 * If this edge is not straight, make it have a symmetrical appearance. Make
	 * the two vectors - from P1 to CTRL1 and from P2 to CTRL2, be of the same
	 * length and have the same angle. So the edge will look it has a
	 * symmetrical curve. There are two cases: The 2 control points are on the
	 * same side of the curve (a curve with the form of a bow); and the 2
	 * control points are on different sides of the edge (a curve like a wave).
	 * In one of the cases, theangles of the vectors should be A=B, in the other
	 * A=-B.
	 */
	protected void symmetrize()
	{
		...
		points[0] = Geometry.translate(curve.getP1(), -curve.getX1(), -curve
				.getY1());
		points[1] = Geometry.translate(curve.getCtrlP1(),
				-curve.getX1(),
				-curve.getY1());
		points[2] = Geometry.translate(curve.getCtrlP2(),
				-curve.getX1(),
				-curve.getY1());
		points[3] = Geometry.translate(curve.getP2(), -curve.getX1(), -curve
				.getY1());

		// rotate all points so edge has angle 0
		float edgeAngle = (float)Math.atan(Geometry.slope(curve.getP1(), curve
				.getP2()));
		...
		float distance = (float)(points[0].distance(points[1]) + points[2]
				.distance(points[3])) / 2F;

		points[1] = Geometry.rotate(new Point2D.Float(distance, 0), (angle
				* (a1 == 0 ? 1 : Math.signum(a1)) + quadrantFix1));
		points[2] = Geometry.rotate(new Point2D.Float(distance, 0), (angle
				* (a2 == 0 ? 1 : Math.signum(a2)) + quadrantFix2 + Math.PI));
	...
	/**
	 * Indicates whether an edge can be rigidly translated with both of its
	 * nodes or must be recomputed. Default value is false. NOT YET USED
	 */
	...
	/**
	 * Set this layout to be the reflection of <code>other</code> where
	 * reflection is about the line between source and target nodes.
	 * 
	 * @param other
	 *            the other layout whose reflection this layout will take
	 */
	...
	/**
	 * Check to see if this layout is the reflection of <code>other</code>,
	 * about the line between source and target nodes.
	 * 
	 * @param other
	 *            the other layout to test against
	...
	public boolean isReflectionOf(BezierLayout other)
	{
		if (this.edge.getSourceNode().equals(other.edge.getSourceNode()))
		{
			return ((this.angle1 == other.angle1 * -1) && (this.angle2 == other.angle2
					* -1));
		}
		else
		{
	 ...
	 * 
	 * @param sourceT
	 *            the parameter of point on parametric cubic curve where my edge
	...
	/**
	 * Returns the point where my edge intersects with the boundary of its
	 * source node
	 * 
	...
	// ///////////////////////////////////////////////////////////////////////
	/*
	 * TODO REMOVE The following methods manage the set of event names to appear
	 * on the edge label. This should be between the edge and its label; this
	 * class should stay out of it.
	 */
