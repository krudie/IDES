...
public abstract class Edge extends GraphElement
{
	 ...
	 *            the target node
	 * @param t
	 *            the transition to be represented by this edge
	 */
	 ...
	 * transition. NOTE No default transition is created since this edge gets
	 * its Id from its first transition.
	 * 
	 * @param source
	 *            the source node
	 * @param target
	 ...
	 * @param type
	 *            SOURCE or TARGET
	 * @return the first point between middle of edge and centre of Node with
	 *         given type where this edge intersects the boundary of
	 *         <code>node</code>, null if no intersection exists.
	 */
	public abstract Point2D intersectionWithBoundary(Node node, int type);

	/**
	 * Sets the handler for this edge with <code>handler</code>. Replace the
	 * current edge handler (if exist) with the <code>handler</code>.
	 * 
	 * @param handler
	 */
	...
	/**
	 * Adds the symbol to the list of event symbols to be displayed on this
	 * edge. TODO event symbols should be passed directly to the edge label
	 * 
	 * @param symbol
	 */
	 ...
	 * 
	 * @param point
	 *            the value to set the point
	 * @param pointType
	 *            a constant indicating which point to set
	 */
	...
	/**
	 * If this edge is not already straight and can be straightened straightens
	 * it.
	 */
