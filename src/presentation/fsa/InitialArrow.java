...
public class InitialArrow extends Edge
{
	...
	/**
	 * point of intersection of direction vector with boundary of target node
	 * shape
	 */
	 ...
	 * 
	 * @param target
	 *            the node representing an initial state
	 */
	 ...
	 * 
	 * @param direction
	 *            a vector meaning the direction of the arrow
	 */
	...
	public void draw(Graphics g)
	{
		...
		if (highlighted || getTargetNode() != null
				&& getTargetNode().isHighlighted())
		{
			setHighlighted(true);
		...
		if (isSelected() || getTargetNode() != null
				&& getTargetNode().isSelected())
		{
	...
	public void computeEdge()
	{
		...
		float smallestArrowSize = CircleNodeLayout.DEFAULT_RADIUS + 2
		...
		line.x1 = sourcePt.x;
		line.y1 = sourcePt.y;
		targetPoint = intersectionWithBoundary(getTargetNode(),
				Edge.TARGET_NODE);
	 ...
	 * 
	 * @param point
	 *            the new value for the point of the given type
	 * @param pointType
	 *            must be of type P1 (? or P2 when Node is moved)
	...
	public void setPoint(Point2D point, int pointType)
	{
		if (pointType == P1)
		{
			...
			/*
			 * TODO Since updating direction vector both in this class and in
			 * NodeLayout get rid of direction field and just use get and
			 * setArrow in NodeLayout
			 */
	...
	/**
	 * Sets my layout to fit among the set of existing edges adjacent to my
	 * target node.
	 * 
	 * @param neighbours
	 *            the set of edges between my source and target nodes
	 */
	...
	/**
	 * FIXME can't use arrow to compute bounds since it is never moved to where
	 * it is drawn (see java.awt.geom.AffineTransform)
	 */
	@Override
	...
	private Point2D.Float computeBestDirection(Node target)
	{
		...
		while (adjEdges.hasNext())
		{
			...
			{
				currentDirVector = Geometry.subtract(target.getLocation(), edge
						.getTargetEndPoint());
				float currentAngle = (float)Geometry
						.angleFrom(currentDirVector, new Point2D.Float(-1, 0));
				angles.add(currentAngle);
		...
		if (canFit)
		{
			return (Geometry.rotate(new Point2D.Float(-1, 0),
					-(float)(2 * Math.PI * 150 / 360)));
		}
	...
	/**
	 * Auto-format: Change the angle of the axis to make the initial arrow go to
	 * the most confortable position
	 */
	...
	private class Handler extends EdgeHandler
	{
		...
		public void refresh()
		{
			...
			anchor = new Ellipse2D.Double(getEdge().getSourceEndPoint().x
					- RADIUS, getEdge().getSourceEndPoint().y - RADIUS, d, d);
