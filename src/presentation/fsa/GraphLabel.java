...
public class GraphLabel extends GraphElement
{
	...
	public void draw(Graphics g)
	{
		...
		if (visible)
		{
			if ((selected || highlighted) && !getText().equals(""))
			{ // && parent != null && !parent.isSelected()){
		...
		if (LatexManager.isLatexEnabled())
		{
			...
			{
				((Graphics2D)g).drawImage(rendered, null, (int)renderedBounds
						.getX(), (int)renderedBounds.getY());
			}
		}
		else
		{
	...
	/**
	 * Draws the text for this label in the given graphics context. Updates
	 * bounds based on font metrics of graphics and the text to be drawn.
	 * 
	 * @param g
	 */
	private void drawText(Graphics g)
	{
		...
		int x = BentoBox.convertDoubleToInt(getLayout().getLocation().x
	...
	private void drawBorderAndTether(Graphics g)
	{
		...
		{ // draw the tether
			...
			Point2D.Double corner = nearestCorner(getParent().getLocation(),
					bounds);
			...
			g.drawLine((int)corner.x, (int)corner.y, (int)getParent()
					.getLocation().x, (int)getParent().getLocation().y);
		}
	...
	public Rectangle bounds()
	{
		...
		if (LatexManager.isLatexEnabled())
		{
			...
			labelBounds.x = BentoBox.convertDoubleToInt(getLayout()
					.getLocation().x
					- (labelBounds.width / DBL_RENDERED_SCALE_WIDTH));
			labelBounds.y = BentoBox.convertDoubleToInt(getLayout()
					.getLocation().y
	...
	public String createExportString(Rectangle selectionBox, int exportType)
	{
		...
		/*
		 * if (getParent() instanceof CircleNode) { CircleNode parentNode =
		 * (CircleNode) getParent(); CircleNodeLayout nodeLayout =
		 * (CircleNodeLayout) parentNode.getLayout(); if
		 * (nodeLayout.getLocation().y < (labelBounds.y + (labelBounds.height /
		 * 2.0))) { labelBounds.y = BentoBox.convertDoubleToInt(
