...
public class Geometry
{
	...
	/**
	 * Returns the angle to to rotate <code>v1</code> such that it is a scalar
	 * of <code>v2</code>.
	 * 
	 * @param v1
	 *            source vector
	 * @param v2
	 *            target vector (within a scalar)
	...
	public static double angleFrom(Point2D.Float v1, Point2D.Float v2)
	{
		...
		/*
		 * a · b = cos ? For non-unit vectors: (1) normalize each vector, (2)
		 * compute the dot product, (3) take the arc cos to get the angle.
		 */
		...
		// DEBUG
		/*
		 * if(java.lang.Double.isNaN(a)) { System.err.println("angle is NaN:
		 * cosA is " + cosA); }
		 */
	 ...
	 */
	public static Point2D.Float unitDirectionVector(Point2D.Float p1,
			Point2D.Float p2)
	{
	...
	/**
	 * The vector perpendicular to <code>v</code> (rotated 90 degrees
	 * clockwise).
	 * 
	 * @param v
	 *            the vector
	...
	/**
	 * Returns the vector resulting from rotating <code>v</code> by
	 * <code>r</code> radians.
	 * 
	 * @param v
	 *            vector
	...
	public static Point2D.Float rotate(Point2D v, double r)
	{
		...
		return new Point2D.Float(
				(float)(v.getX() * c - v.getY() * s),
				(float)(v.getY() * c + v.getX() * s));
	}
	 ...
	 * 
	 * @param v
	 *            vector with origin at (0,0) and given direction
	 * @param s
	 *            the scalar
	...
	/**
	 * Given two points, find the slope (Y2-Y1)/(X2-X1) of the line between
	 * them.
	 * 
	 * @param p1
	 *            point 1
