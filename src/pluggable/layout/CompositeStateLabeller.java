...
public class CompositeStateLabeller {
	...
	public static void labelStates(FSAModel fsa)
	{
		...
		{
			FSAModel[] gs=new FSAModel[((String[])fsa.getAnnotation(Annotable.COMPOSED_OF)).length];
			for(int i=0;i<gs.length;++i)
			{
				DESModel m=Hub.getWorkspace().getModelById(((String[])fsa.getAnnotation(Annotable.COMPOSED_OF))[i]);
			...
			for(Iterator<FSAState> si=fsa.getStateIterator();si.hasNext();)
			{
				...
				subState=gs[gs.length-1].getState(s.getStateCompositionList()[gs.length-1]);
				if(subState==null)
				{
					return;
				}
		...
		{
			DESModel m=Hub.getWorkspace().getModelById(((String[])fsa.getAnnotation(Annotable.COMPOSED_OF))[0]);
			if(m==null||!(m instanceof FSAModel))
			{
				return;
			}
			...
			for(Iterator<FSAState> si=fsa.getStateIterator();si.hasNext();)
			{
				...
				if(s.getStateCompositionList().length>1)
				{
					...
					subState=g.getState(s.getStateCompositionList()[s.getStateCompositionList().length-1]);
					if(subState==null)
					{
						return;
					}
				...
				{
					FSAState subState=g.getState(s.getStateCompositionList()[0]);
					if(subState==null)
					{
						return;
					}
