...
/**
 * The manager of models available to IDES. This allows for custom model types
...
public class ModelManager
{
	...
	/**
	 * Index of all registered {@link ModelDescriptor}s. The key is the DES
	 * model interface implemented by the model. The value is the set of the
	 * model descriptors of all models implementing the given interface. Note
	 * that if a model implements a number of interfaces, its model descriptor
	...
	/**
	 * Returns the model descriptors for all models that implement the given
	 * interface.
	 * 
	 * @param iface
	 *            the interface for which model descriptors should be returned
	...
	public static ModelDescriptor[] getAllModels()
	{
		Set<ModelDescriptor> all = new TreeSet<ModelDescriptor>(
				new Comparator<ModelDescriptor>()
				{
					...
					{
						return md1.getTypeDescription().compareTo(md2
								.getTypeDescription());
					}
				});
		for (Enumeration<Set<ModelDescriptor>> i = class2Model.elements(); i
				.hasMoreElements();)
		{
	...
	/**
	 * Registers a model descriptor so that the corresponding DES model becomes
	 * available to IDES.
	 * 
	 * @param md
	 *            the model descriptor for the DES model
	 */
