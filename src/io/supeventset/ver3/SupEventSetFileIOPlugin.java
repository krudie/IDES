...
public class SupEventSetFileIOPlugin implements FileIOPlugin
{
			...
			throws FileSaveException
	{
		try
		{
			Iterator<SupervisoryEvent> ei = ((SupervisoryEventSet)model)
					.iteratorSupervisory();
			while (ei.hasNext())
			{
...

	{
		...
		protected final String OBSERVABLE = "observable",
				CONTROLLABLE = "controllable", NAME = "name", ID = "id";
		...
		/**
		 * Parses XML information and builds a SupervisoryEventSet based on the
		 * content given by the IOCoordinator
		 * 
		 * @param qName
		...
		/**
		 * Create an Event based on the given atts. Since it returns an Object,
		 * the function is expandable to return any model element which may be
