		...
		while (buffer.length() > 0)
		{
			...
			case MODEL_INFO_S:
				if (buffer.length() >= MODEL.length()
						&& MODEL.equals(buffer
								.substring(0, MODEL.length()).toLowerCase()))
				{
			...
			case MODEL_INFO_E:
				if (buffer.length() >= CLOSE.length()
						&& CLOSE.equals(buffer.substring(0, CLOSE.length())))
				{
			...
			case DATA_S:
				if (buffer.length() >= DATA.length()
						&& DATA.equals(buffer
								.substring(0, DATA.length()).toLowerCase()))
				{
			...
			case DATA_E:
				if (buffer.length() >= DATA_CLOSE.length()
						&& DATA_CLOSE.equals(buffer.substring(0,
								DATA_CLOSE.length()).toLowerCase()))
				{
					if (nestedElements == 0)
					{
				...
				break;
			case META_S:
				if (buffer.length() >= META.length()
						&& META.equals(buffer
								.substring(0, META.length()).toLowerCase()))
				{
					element = new StringBuffer();
					element.append(buffer.charAt(0));
					mode = META_SE;
			...
			case META_E:
				if (buffer.length() >= META_CLOSE.length()
						&& META_CLOSE.equals(buffer.substring(0,
								META_CLOSE.length()).toLowerCase()))
				{
					if (nestedElements == 0)
					{
				...
				else if ((buffer.length() >= META.length() && META
						.equals(buffer
								.substring(0, META.length()).toLowerCase()))
						|| (buffer.length() >= META_SIMPLE.length() && META_SIMPLE
								.equals(buffer.substring(0,
										META_SIMPLE.length()).toLowerCase())))
				{
					nestedElements++;
				}
				break;
