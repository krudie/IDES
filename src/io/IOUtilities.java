...
public class IOUtilities
{
	 ...
	 * 
	 * @param file
	 *            the file that needs a printstream wrapped around it
	 * @return the printstream pointing to a the file, <code>null</code> if it
	...
	public static PrintStream getPrintStream(File file)
	{
		...
		try
		{
			if (!file.exists())
			{
				...
				{
					errorMsg = Hub.string("fileUnableToCreate")
							+ file.getPath();
	...
	/**
	 * Class used to create file filters for Open/Save/etc. dialog boxes. It
	 * filters files depending on their extenstions.
	 * 
	 * @author Lenko Grigorov
	 */
	public static class ExtensionFilter extends FileFilter
	{
		...
		/**
		 * Construct a new file filter which will select all directories
		 * (regardless of name) and all files having an extension equal to the
		 * provided extension (regardless of letter case).
		 * 
		 * @param extension
		 *            file extension to be used for filtering (e.g., "txt")
		 * @param description
		 *            description to be used for the filter (e.g., "Text file")
		 */
		...
		/**
		 * Construct a new file filter which will select all directories
		 * (regardless of name) and all files having an extension equal to on of
		 * the provided extensions (regardless of letter case).
		 * 
		 * @param extensions
		 *            file extensions to be used for filtering (e.g.,
		 *            ["jpg","bmp","png"])
		 * @param description
		 *            description to be used for the filter (e.g., "Image file")
		 */
		 ...
		 * 
		 * @return <code>true</code> if the file is a directory or if the
		 *         extension of the file is one of the accepted extensions
		 *         (regrdless of letter case), otherwise returns
		 *         <code>false</code>
		 */
		@Override
		public boolean accept(File f)
		{
			...
			{
				extension = f
						.getName().substring(f.getName().lastIndexOf('.') + 1);
			}
