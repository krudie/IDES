...
public class TemplateFileIOPlugin implements FileIOPlugin
{
	...
	public void initializeFileIO()
	{
		...
		IOPluginManager
				.getInstance().registerDataLoader(this, "TemplateDesign");
			...
			File fileDirectory)
	{
		...
		// The following commented code is the code written by Lenko which saves
		// the
		...
		// {
		// stream.println("\t<link lefttype=\""+(l.getLink().getBlockLeft()
		// instanceof TemplateModule?"m":"c")+
		// "\" leftblock=\""+l.getLink().getBlockLeft().getId()+
		// "\" leftevent=\""+l.getLink().getEventLeft().getId()+
		// "\" righttype=\""+(l.getLink().getBlockRight() instanceof
		// TemplateModule?"m":"c")+
	...
	public static class XMLExporter
	{
		...
		public static void graph2XML(TemplateGraph graph, File dir)
		{
			...
			for (GraphBlock b : graph.getBlocks())
			{
				...
				try
				{
					...
					ps = IOUtilities.getPrintStream(new File(dir
							.getCanonicalPath()
							+ File.separator
							+ fsa.getName()
							+ "."
					...
					if (ps == null)
					{
						throw new RuntimeException(Hub
								.string("checkInvalidChars"));
					}
					// TODO: MAKE THE XML, EXPORTER BE AN INNER CLASS, LIKE IT
					// IS DONE IN THE FSAFileIOPlugin
			...
			try
			{
				ps = IOUtilities.getPrintStream(new File(dir.getCanonicalPath()
						+ File.separator + "TemplateDesign."
						+ IOUtilities.MODEL_FILE_EXT));
				if (ps == null)
				{
		...
		public static void export(TemplateGraph graph, PrintStream ps)
		{
			...
			for (GraphBlock b : graph.getBlocks())
			{
				if (b.getBlock() instanceof TemplateModule)
				{
					ps.println("\t<module fsa=\""
							+ b.getBlock().getFSA().getName() + "\" id=\""
							+ b.getBlock().getId() + "\">");
					for (FSAEvent event : b.getBlock().getInterfaceEvents())
					{
			...
			{
				ps
						.println("\t<link lefttype=\""
								+ (l.getLink().getBlockLeft() instanceof TemplateModule ? "m"
										: "c")
								+ "\" leftblock=\""
								+ l.getLink().getBlockLeft().getId()
								+ "\" leftevent=\""
								+ l.getLink().getEventLeft().getId()
								+ "\" righttype=\""
								+ (l.getLink().getBlockRight() instanceof TemplateModule ? "m"
										: "c") + "\" rightblock=\""
								+ l.getLink().getBlockRight().getId()
								+ "\" rightevent=\""
								+ l.getLink().getEventRight().getId()
								+ "\" id=\"" + l.getLink().getId() + "\"/>");
			}
			Map<String, String> codeMap = graph.getModel().getPLCCodeMap();
			for (String event : codeMap.keySet())
			...
			for (GraphBlock b : graph.getBlocks())
			{
				if (b.getBlock() instanceof TemplateModule)
				{
					ps.println("\t<module x=\"" + b.getLocation().x + "\" y=\""
							+ b.getLocation().y + "\" name=\"" + b.getName()
							+ "\" id=\"" + b.getBlock().getId() + "\"/>");
				}
				else
				// Template Channel
				{
					ps.println("\t<channel x=\"" + b.getLocation().x
							+ "\" y=\"" + b.getLocation().y + "\" name=\""
							+ b.getName() + "\" id=\"" + b.getBlock().getId()
							+ "\"/>");
				}
			}
			ps.println("</meta>");
