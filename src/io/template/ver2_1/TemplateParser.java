...
public class TemplateParser
{
	...
	{
		TemplateModel model = ModelManager.createModel(TemplateModel.class,
				ParsingToolbox.removeFileType(f.getName()));
		...
		try
		{
			...
			{ // read "data" section
				...
				{ // read PLC code
					...
					String code = tag
							.getChildNodes().item(0).getTextContent().trim()
				...
				{ // read Links
					...
					TemplateBlock left = leftIsModule ? model
							.getModule(leftBlock) : model.getChannel(leftBlock);
					TemplateBlock right = rightIsModule ? model
							.getModule(rightBlock) : model
							.getChannel(rightBlock);
					TemplateLink link = new Link(id, left, left
							.getFSA().getEvent(leftEvent), right, right
							.getFSA().getEvent(rightEvent));
					model.add(link);
				}
				else
				{ // read Modules and Channels
					...
					for (int j = 1; j < events.getLength(); ++j)
					{
						...
						ifaceIds.add(new Long(Long.parseLong(attribute
								.getNodeValue())));
					}
					File fsaFile = new File(f.getParentFile().getAbsolutePath()
							+ File.separator + fsaName + "."
					...
					try
					{
						fsa = (FSAModel)IOCoordinator
								.getInstance().load(fsaFile);
					}
					catch (Exception e)
					{
						throw new RuntimeException(Hub
