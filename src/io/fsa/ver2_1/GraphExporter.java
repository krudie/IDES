...
public class GraphExporter
{
	...
	{
		private JCheckBox cbFrame = new JCheckBox(Hub
				.string("addFrameToExport"));
			...
			PrintStream ps)
	{
		...
		/*
		 * exportBounds.width -= exportBounds.x; exportBounds.height -=
		 * exportBounds.y;
		 */
		border = (exportBounds.x > INT_PSTRICKS_BORDER_SIZE) ? INT_PSTRICKS_BORDER_SIZE
				: exportBounds.x;
		exportBounds.x -= border;
		exportBounds.width += (border * 2);

		border = (exportBounds.y > INT_PSTRICKS_BORDER_SIZE) ? INT_PSTRICKS_BORDER_SIZE
				: exportBounds.y;
		...
		{
			scale = (scale > (DBL_PSTRICKS_MAX_HEIGHT / exportBounds.height)) ? DBL_PSTRICKS_MAX_HEIGHT
					/ exportBounds.height
					: scale;
		}
			...
			Rectangle exportBounds, boolean useFrame)
	{
		...
		for (int i = 0; i < freelabelArray.length; i++)
		{
			contentsString += freelabelArray[i]
					.createExportString(exportBounds, INT_EXPORT_TYPE_PSTRICKS);
		}
		contentsString += "\n";
	 ...
	 * necessary for the entire graph. It goes through every node, edge and
	 * label and uss the min and max x and y values in these to create the box.
	 * 
	 ...
	 * exportBounds.checkRectangle(edgeArray[i].getLabel().bounds()); } for (i =
	 * 0; i < labelArray.length; i++) { // TODO: Get better edge label bounds!!!
