...
public class CommonFileActions
{
	...
	public static void open()
	{
		...
		if (retVal == JFileChooser.APPROVE_OPTION)
		{
			...
			if (Hub.getWorkspace().getModel(ParsingToolbox.removeFileType(fc
					.getSelectedFile().getName())) != null)
			{
	...
	public static void importFile()
	{
		...
		if (retVal == JFileChooser.APPROVE_OPTION)
		{
			...
			try
			{
				// calling IOCoordinator to handle the selected file
				// It will make the correct plugins load the file):
				model = IOCoordinator.getInstance().importFile(file,
						fc.getFileFilter().getDescription());
			}
			catch (Exception e)
			{
	 ...
	 * @param file
	 *            the file where the workspace will be written
	 ...
	 * 
	 * @param file
	 *            the file containing the workspace description
	 ...
	 * 
	 * @param wd
	 *            the workspace descriptor to convert to XML
	 * @param ps
	 *            the printstream this object should be printed to.
	 */
	 ...
	 * 
	 * @param m
	 *            the DESModel that needs to be saved
	 * @return false if the process was cancelled
	 */
	public static boolean handleUnsavedModel(DESModel m)
	{
		...
		if (saveChoice != JOptionPane.YES_OPTION
				&& saveChoice != JOptionPane.NO_OPTION)
		{
			return false;
		}
		if (saveChoice == JOptionPane.YES_OPTION)
	...
	public static boolean handleUnsavedModels(Vector<DESModel> models)
	{
		if (models.size() > 0)
		{
			Vector<DESModel> selectedModels = new SaveDialog(models)
					.selectModels();
