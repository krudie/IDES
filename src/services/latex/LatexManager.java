...
public class LatexManager
{
	...
	/**
	 * A {@link Runnable} that toggles the LaTeX redering on. This is needed
	 * since the {@link LatexPrerenderer} displays its progress; thus the
	 * updating cannot be done inside the Swing event loop.
	 * 
	...
	private static class SetLatexUpdater implements Runnable
	{
		...
		public void run()
		{
			if (new LatexPrerenderer(Hub
					.getWorkspace().getLayoutShellsOfType(FSAGraph.class)
					.iterator()).waitFor())
			{
