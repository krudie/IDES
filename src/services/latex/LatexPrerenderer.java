...
public class LatexPrerenderer extends InterruptableProgressDialog
{
	...
	/**
	 * The {@link LatexPresentation}s whose labels should be rendered. The key
	 * is the {@link LatexPresentation} and the value is the set of
	...
	/**
	 * Displays a dialog box with a progress bar and starts rendering the labels
	 * of a {@link LatexPresentation}. The user may cancel the process using the
	 * controls in the dialog box.
	 * 
	 * @param lp
	 *            the {@link LatexPresentation} whose labels have to be rendered
	 */
	...
	/**
	 * Displays a dialog box with a progress bar and starts rendering the labels
	 * of a set of {@link LatexPresentation}s. The user may cancel the process
	 * using the controls in the dialog box.
	 * 
	 * @param lps
	 *            an iterator over the set of {@link LatexPresentation}s whose
	...
	public LatexPrerenderer(Iterator<LatexPresentation> lps)
	{
		...
		for (; lps.hasNext();)
		{
			...
			{
				Collection<LatexElement> elements = lp
						.getUnrenderedLatexElements();
				if (!elements.isEmpty())
				{
					toPrerender.put(lp, elements);
				}
	...
	public void run()
	{
		...
		for (LatexPresentation model : toPrerender.keySet())
		{
			...
			label.setText(Hub.string("renderPrerender")
