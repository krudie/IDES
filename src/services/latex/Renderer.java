...
public class Renderer
{
	...
	/**
	 * Used to call external programs from a separate thread so that timeout can
	 * be implemented.
	 * 
	...
	private class Executor extends Thread
	{
		 ...
		 * 
		 * @param cmd
		 *            the command+parameters that will be called
		 * @param dir
		 *            the startup directory for the new process
		 */
		...
		/**
		 * Executes the call as specified during the construction of this
		 * object. After the call completes, <code>done</code> is set to true
		 * and all threads waiting on this Executor are awoken. The thread can
		 * be interrupted (via {@link java.lang.Thread#interrupt()}) in case of
		 * a timeout; if this happens, the call is aborted.
		 */
		@Override
		public void run()
		{
		...
		/**
		 * Returns <code>true</code> if the the execution of the call has
		 * completed and <code>false</code> otherwise.
		 * 
		 * @return <code>true</code> if the the execution of the call has
		...
		/**
		 * Returns the command (without parameters) wich this object will
		 * execute. This can be used in announcements in case of timeouts.
		 * 
	 ...
	 * 
	 * @param latexPath
	 *            the directory where the <code>latex</code> and
	 *            <code>dvips</code> binaries are located (e.g.,
	 *            "c:\texmf\miktex\bin")
	 * @param gsPath
	 *            the location of the GhostScript command-line engine (e.g.,
	 ...
	 */
	public static Renderer getRenderer(java.io.File latexPath,
			java.io.File gsPath)
	{
	 ...
	 *             if there was a problem while rendering the LaTeX code
	 ...
	 */
	public BufferedImage renderString(String s, File f) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderString(String s, OutputStream out)
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderString(String s, String header, File f)
			throws IOException, LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(String doc, File f) throws IOException,
			LatexRenderException
	{
	...
	/**
	 * Render the first page of a LaTeX document into a PNG image and output to
	 * a stream.
	 * 
	 * @param doc
	 *            LaTeX document
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(String doc, OutputStream out)
			throws IOException, LatexRenderException
	{
		BufferedImage im = renderDocument(doc);
	 ...
	 * 
	 * @param doc
	 *            file containing the LaTeX document
	 * @param f
	 *            file where to save image. If the file exists, its content will
	 *            be overwritten.
	 * @return the image which was saved in the file
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(File doc, File f) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(File doc, OutputStream out)
			throws IOException, LatexRenderException
	{
	 ...
	 * 
	 * @param doc
	 *            InputStream from which the LaTeX document will be read. The
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(InputStream doc, File f)
			throws IOException, LatexRenderException
	{
		BufferedImage im = renderDocument(doc);
		ImageIO.write(im, "png", f);
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(InputStream doc, OutputStream out)
			throws IOException, LatexRenderException
	{
		BufferedImage im = renderDocument(doc);
		ImageIO.write(im, "png", out);
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderString(String s) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderString(String s, String header)
			throws IOException, LatexRenderException
	{
		return renderDocument("\\documentclass{article}\n"
				+ header
	 ...
	 * @see #renderDocument(InputStream)
	 */
	public BufferedImage renderDocument(String doc) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(File doc) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #renderDocument(String)
	 */
	public BufferedImage renderDocument(InputStream doc) throws IOException,
			LatexRenderException
	{
		File latexFile = File.createTempFile("ides", ".tex");
		BufferedOutputStream out = new BufferedOutputStream(
				new FileOutputStream(latexFile));
	...
	/**
	 * Render the first page of the LaTeX document contained in a temporary
	 * file. This method is called by all other rendering methods. The method
	 * removes all temporary files (including the file in the parameter) after
	 ...
	 * @see #renderDocument(String)
	 */
	protected BufferedImage renderTempFile(File latexFile) throws IOException,
			LatexRenderException
	{
	 ...
	 *            file containing the LaTeX document
	 * @param dpi
	 *            the resolution to be used for the redering (in dots per inch)
	 ...
	 */
	protected BufferedImage latex2PNG(File latexFile, int dpi)
			throws IOException, LatexRenderException
	{
		String[] command;
		...
		Executor latex = new Executor(command, new File(latexFile
				.getParentFile().getCanonicalPath()));
		execute(latex);

		// Check if LaTeX output anything
		File latexOutputFile = new File(latexFile
				.getParentFile().getCanonicalPath()
				+ File.separator
				+ latexFile.getName().substring(0,
						latexFile.getName().lastIndexOf('.')) + ".dvi");
		...
		command[2] = "-q";
		command[3] = "-o";
		command[4] = latexFile.getName().substring(0,
				latexFile.getName().lastIndexOf('.'))
				+ ".ps";
		command[5] = latexFile.getName().substring(0,
				latexFile.getName().lastIndexOf('.'))
				+ ".dvi";
		Executor dvips = new Executor(command, new File(latexFile
				.getParentFile().getCanonicalPath()));
		execute(dvips);
		...
		command[6] = "-r" + dpi;
		command[7] = "-sOutputFile="
				+ latexFile.getName().substring(0,
						latexFile.getName().lastIndexOf('.')) + ".png";
		command[8] = latexFile.getName().substring(0,
				latexFile.getName().lastIndexOf('.'))
				+ ".ps";
		Executor gs = new Executor(command, new File(latexFile
				.getParentFile().getCanonicalPath()));
		execute(gs);

		File pngFile = new File(latexFile.getCanonicalPath().substring(0,
				latexFile.getCanonicalPath().lastIndexOf('.'))
				+ ".png");
		...
		if (in.read(pngData, 0, pngData.length) != pngData.length)
		{
			throw new IOException(
					"Unable to completely read intermediate image file.");
		}
		in.close();
		...
		BufferedImage im = new BufferedImage(tempIm.getWidth(null), tempIm
	...
	/**
	 * Deletes all files which are located in the same directory as the
	 * parameter and which have the same name (only the extension can differ).
	 * <p>
	 * For example, if the parameter is the file "c:\temp\ides0013.tex", then
	 * the files "c:\temp\ides0013.*" will be deleted.
	 * 
	 * @param latexFile
	 *            file whose name and location will be used to determine which
	 *            files to delete
	 * @see #renderTempFile(File)
	 * @see #tempLatex2EPS(File, File)
	 */
	private void removeTempFiles(File latexFile)
	{
		...
		for (int i = 0; i < files.length; ++i)
		{
			...
			if (files[i]
					.substring(0, files[i].lastIndexOf('.')).equals(latexFile
							.getName().substring(0,
									latexFile.getName().lastIndexOf('.'))))
			{
				try
				{
					new File(latexFile.getParentFile().getCanonicalPath()
							+ File.separator + files[i]).delete();
				}
				catch (IOException e)
				{
				}
	 ...
	 * 
	 * @param ex
	 *            the {@link Renderer.Executor} with the call that has to be
	 *            executed
	 * @throws LatexRenderException
	 *             if the call timed out or if the call didn't succeed
		...
		synchronized (ex)
		{
			if (!ex.isDone())
			{
				try
				{
					...
					{
						ex.interrupt();
						throw new LatexRenderException("The invocation of "
								+ ex.getCommand() + " timed out.");
					}
				}
				catch (InterruptedException e)
				{
	 ...
	 *            LaTeX document
	 * @param target
	 *            file where to save the EPS. If the file exists, its content
	 *            will be overwritten.
	 * @throws IOException
	 *             if there was a problem with the IO
	 * @throws LatexRenderException
	 ...
	 */
	public void latex2EPS(String doc, File target) throws IOException,
			LatexRenderException
	{
		File latexFile = File.createTempFile("ides", ".tex");
		FileWriter out = new FileWriter(latexFile);
	 ...
	 */
	public void latex2EPS(File doc, File target) throws IOException,
			LatexRenderException
	{
	 ...
	 * @see #latex2EPS(String, File)
	 */
	public void latex2EPS(InputStream doc, File target) throws IOException,
			LatexRenderException
	{
		File latexFile = File.createTempFile("ides", ".tex");
		BufferedOutputStream out = new BufferedOutputStream(
	...
	/**
	 * Convert the first page of a LaTeX document contained in a temporary file
	 * into an EPS file. This method is called by all other methods that convert
	 * into EPS. The method removes all temporary files (including the file in
	 * the parameter) after finishing conversion.
	 * 
	 * @param latexFile
	 *            file containing the LaTeX document
	 * @param target
	 ...
	 * @see #latex2EPS(String, File)
	 */
	protected void tempLatex2EPS(File latexFile, File target)
			throws IOException, LatexRenderException
	{
		BufferedImage im = null;
		try
		{
			...
			// the page in EPS
			fixBoundingBox(r, new File(latexFile
					.getCanonicalPath().substring(0,
							latexFile.getCanonicalPath().lastIndexOf('.'))
					+ ".ps"), target);
		}
		finally
	...
	/**
	 * Fixes the <code>BoundingBox</code> comment in EPS files so it reflects
	 * the true boundary of the graphic. Some tools (notably <code>dvips</code>)
	 * may not always generate a correct <code>BoundingBox</code> comment.
	 * 
	 * @param boundary
	 *            the correct boundary of the graphic
	 * @param source
	 *            EPS file which needs to be fixed. Cannot be the same as the
	 *            target file.
	 * @param target
	 *            file where the corrected EPS will be written. Cannot be the
	 *            same as the source file.
	 * @throws IOException
	 *             if there was a problem with the IO
	 * @see #tempLatex2EPS(File, File)
	 */
	protected void fixBoundingBox(Rectangle boundary, File source, File target)
			throws IOException
	{
		...
		while ((b = in.read()) >= 0) // loop to find "%%BoundingBox:"
		{
			...
			if (b == 10) // if new line
			{
				...
				// string was matched
				{
					...
					out.write(("%%BoundingBox: " + boundary.x + " "
							+ boundary.y + " " + (boundary.x + boundary.width)
							+ " " + (boundary.y + boundary.height) + (char)10)
							.getBytes());
