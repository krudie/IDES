...
public class CopyPasteBackend implements CopyPasteManager
{
	...
	/**
	 * Initializes the CopyPasteBackend. The CopyPasteBackend subscribes itself
	 * to the workspace, so it can proccess events for model switching and
	 * similar tasks.
	 */
	...
	/**
	 * Determines whether the cut and copy options should be available to the
	 * user. First checks whether the activeCopyPastePresentation is not null,
	 * then calls isCutCopyEnabled() on the presentation.
	 * 
	...
	/**
	 * Determines whether the paste option should be available to the user.
	 * First checks whether the activeCCPPresentatino is not null, then calls
	 ...
	 * other presentations call this method. If the clipboard contents have
	 * changed, calls newItemOnClipboard on the active CopyPastePresentation.
	 */
	public void refresh()
	{
