...
public class CacheBackend implements Cache
{
	...
	/**
	 * This determines when files will be purged from the cache. If an item in
	 * the cache has not been used for longer than this period of time (in
	 * seconds), it will be deleted when the cache is closed.
	 * 
	 * @see #close()
	 */
	...
	{
		System
				.setProperty("org.apache.commons.logging.simplelog.log.com.opensymphony.oscache",
						"error");
		...
		cacheConfig
				.put("cache.persistence.class",
						"com.opensymphony.oscache.plugins.diskpersistence.HashDiskPersistenceListener");
		cacheConfig.put("cache.path", CACHE_DIR);
		cache = new com.opensymphony.oscache.general.GeneralCacheAdministrator(
				cacheConfig);
		// insert empty LaTeX label
		try
		{
			cache.putInCache(GraphLabel.class.getName(), LatexUtils
					.labelStringToImageBytes(""));
		}
	 ...
	 * 
	 * @param key
	 *            the key that will be used for retrieval
	 * @param value
	 ...
	 *             when the object is not found in the cache
	...
	public Object get(String key) throws NotInCacheException
	{
		...
		{
			Object value = cache
					.getFromCache(key, CacheEntry.INDEFINITE_EXPIRY);
			// refresh
			cache.putInCache(key, value);
			return value;
		}
		catch (NeedsRefreshException e)
		{
			...
			throw new NotInCacheException("Can't find object for the key: "
					+ key);
		}
	}
	...
	{
		File[] cachedFiles = new File(CACHE_DIR + File.separator
