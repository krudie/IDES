...
public class NoticeBoard extends JScrollPane implements MouseListener, MouseMotionListener, ChangeListener
{
	...
	/**
	 * The UI element of the notice which was last clicked.
	 * This info is needed for a nasty hack to achieve auto-scrolling.
	 */
	protected NoticeUI focused=null;
	/**
	 * The height of the UI element of the notice which was last clicked,
	 * at the time of clicking it.
	 * This info is needed for a nasty hack to achieve auto-scrolling.
	 */
	protected int focusedLastHeight=0;

	/**
	 * Responds to changes in the scroll pane containing the notices.
	 * This callback method is needed for a nasty hack to achieve auto-scrolling.
	 */
	...
	/**
	 * Initiates de-highlighting of all notices if the mouse cursor exits the notice board.
	 */
	...
	/**
	 * Returns the UI element of the notice displayed at the given location on the screen. If there is
	 * no notice at this location, returns <code>null</code>.  
	 * @param p a point on the screen
	 * @return the notice displayed at the given location, or <code>null</code> if there
	 * is no notice at the location
	 */
	...
	/**
	 * When run, highlights the notice displayed at the given location on the screen and
	 * un-highlights all other notices. If there is no notice at the given location,
	 * all notices get un-highlighted.
	 * <p>A method cannot be used since the operation has to be performed within the
	 * Swing thread. 
	 */
	...
	/**
	 * When run, toggles the "expanded" state of the notice at the given location on the
	 * screen. If there is no notice at the given location, it does not do
	 * anything.
	 * <p>A method cannot be used since the operation has to be performed within the
	 * Swing thread. 
	 */
