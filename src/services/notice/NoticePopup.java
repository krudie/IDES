 ...
 * {@link NoticeManager}. The balloon is actually always visible but has 0 dimensions
 * when there are no messages to display. "Always on" is necessary to prevent
 * stealing the focus of the main window when the balloon is made visible. 
...
public class NoticePopup extends JDialog implements WindowListener, ComponentListener, MouseListener
{
	 ...
	 * Needed for a nasty hack not to steal focus from the main window.
	 */
	private Object sync=new Object();
	/**
	 * Set to <code>true</code> when the pop-up balloon is becoming visible.
	 * Needed for a nasty hack not to steal focus from the main window.
	 */
	...
	/**
	 * Defines the number of milliseconds after which the popup of a notice should expire.
	 */
	...
	/**
	 * Keeps track of the latest time the user closed the pop-up balloon.
	 * Notices posted earlier than this time will not be displayed in the balloon.
	 */
	...
	/**
	 * Initializes the NoticePopup. Should be called only once
	...
	/**
	 * When the main window is activated (e.g., user brings it to the foreground), also
	 * make the pop-up balloon visible. 
	 */
	...
	public void windowDeactivated(WindowEvent arg0)
	{
		synchronized(sync)
		{
			// nasty hack. When the pop-up balloon is made visible, it steals the
	...
	/**
	 * When the main window is opened, also make the pop-up balloon visible. 
	 */
	...
	/**
	 * When the main window is shown, also make the pop-up balloon visible. 
	 */
	...
	public void setVisible(boolean b)
	{
		...
		else if(!this.isVisible())
		{
			synchronized(sync)
			{
				...
				super.setVisible(b);
				SwingUtilities.invokeLater(new Runnable(){public void run(){Hub.getMainWindow().requestFocus();}});
			}
		}
	}
	
	/**
	 * Updates the content of the pop-up balloon. New notices are added, expired notices
	 * are removed.
	 */
	public void update()
	{
	 ...
	 * If the user does not click on the close button of the balloon, the {@link NoticeBoard}
	 * with all the notices is activated in the main window.
	 */
