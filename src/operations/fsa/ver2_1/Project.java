...
public class Project
{
	 ...
	 *            will be created.
	 * @param removeNulls
	 *            a flag to determine whether or not to project out the epsilon
	 *            (<code>null</code>) transitions in addition to the
	 *            DESEventsToRemove. When <code>true</code>, the null events
	 *            will be projected out, when <code>false</code>, they will not.
		...
		while (!searchList.isEmpty())
		{
			...
			while (ei.hasNext())
			{
				...
				if (!states.isEmpty())
				{
					...
					{
						t = projection
								.assembleEpsilonTransition(source.getId(),
										target.getId());
					}
					else
					{
						t = projection.assembleTransition(source.getId(),
								target.getId(),
								event.getId());
					}
	 ...
	 * events that have been specified to be removed. If removeNulls is true,
	 * the reach will include states accessible through null events. Otherwise,
	 * only those specified in the event set will be removed.
	 * 
	 * @param sll
	 *            the stateset (linked list of states) to use.
	 * @param eventsToRemove
	 *            the events to be removed from the model
	 * @param removeNulls
			...
			DESEventSet eventsToRemove, boolean removeNulls)
	{
		...
		while (sli.hasNext())
		{
			...
			while (stli.hasNext())
			{
				...
				if (!removeNulls)
				{
					if (eventsToRemove.contains(t.getEvent())
				...
				{
					if ((t.getEvent() == null || eventsToRemove.contains(t
							.getEvent()))
							&& !reached.contains(t.getTarget()))
					{
						reached.add(t.getTarget());
	...
	/**
	 * makes a new state from a linked list of states. Doesn't include states
	 * with no label, since these are considered "dummy" states. The name of the
	 * new state comes from the names of the composing states. For example, if
	 * the composing states are 1,2 and 3, the name of the new state will be
	 * (1,2,3). "dummy" states are not included in the name of the new state. If
	 * any of the composing states is marked, the new state will also be marked.
	 * 
	 * @param sll
	 *            the state set to make a new state from
	 * @param finalState
	 *            a state pre-assembled in the desired model that will be
	 ...
	 */
	private static FSAState makeState(LinkedList<FSAState> sll,
			FSAState aState, long id, boolean initial)
	{
		ListIterator<FSAState> sli = sll.listIterator();
		FSAState s;
	...
	/**
	 * private function for setting a linkedlist representing a state in the
	 * model
	 * 
	 * @param sll
	...
	/**
	 * Makes a string of the state ids in a stateset. Doesn't include states
	 * with no label, since these are considered "dummy" states.
	 * 
	 * @param sll
