			...
			while (sti0.hasNext())
			{
				...
				while (sti1.hasNext())
				{
									...
									.getEvent().equals(t1.getEvent())))
					{
						...
						if (id != -1)
						{
							product.add(product.assembleTransition(source
									.getId(), id, event.getId()));
						}
						else
						{
							...
							product.add(target);
							product.add(product.assembleTransition(source
									.getId(), target.getId(), event.getId()));
		...
		while (sia.hasNext())
		{
			...
			if (initial[0].isInitial())
			{
				...
				while (sib.hasNext())
				{
					...
					{
						searchList.add(initial.clone());
						parallel.add(makeState(initial, stateNumber, parallel
								.assembleState()));
						setStateId(initial, stateNumber++);
					}
				}
			...
			for (int i = 0; i < 2; i++)
			{
				...
				while (stli.hasNext())
				{
							...
							|| !intersection.contains(events.get(t.getEvent())))
					{
						...
						if (id != -1)
						{
							parallel.add(parallel.assembleTransition(source
									.getId(), id, event.getId()));
						}
						else
						{
							FSAState target = makeState(s,
									stateNumber,
									parallel.assembleState());
							parallel.add(target);
							parallel.add(parallel.assembleTransition(source
									.getId(), target.getId(), event.getId()));
							setStateId(s, stateNumber++);
