...
public class MultiAgentProductFSA extends AbstractOperation {
	...
	public Object[] perform(Object[] inputs) {
		...
		for(int i=0;i<inputs.length;++i)
		{
			...
			initial[i]=null;
			for(Iterator<FSAState> j=((FSAModel)inputs[i]).getStateIterator();j.hasNext();)
			{
		...
		while(!openStates.isEmpty())
		{
			...
			PseudoTransition pt=computeTransitions(new FSAEvent[0],eventSets,states);
			...
			for(;ei.hasNext();)
			{
				...
				FSATransition transition=new Transition(a.getTransitionCount(),state,target,event);
				a.add(transition);
	...
	protected PseudoTransition computeTransitions(FSAEvent[] selectedEvents,FSAEventSet[] freeEvents, FSAState[] origin)
	{
		...
		for(FSAEvent e:freeEvents[0])
		{
			...
			for(int i=0;i<selectedEvents.length;++i)
			{
				...
				{
					if(hasOutgoingTransitionOn(origin[i],e)||hasOutgoingTransitionOn(origin[selectedEvents.length],selectedEvents[i]))
					{
						includeEvent=false;
						break;
					}
