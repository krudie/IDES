...
public class SuperVisory
{
	 ...
	 *            The legal language
	 * @param result
	 *            An empty automaton to use for the result
	 */
		...
		while (changed)
		{
			...
			while (si.hasNext())
			{
				...
				while (plsti.hasNext())
				{
					...
					if (plst.getEvent() != null
							&& !plst.getEvent().isControllable())
					{
						ListIterator<FSATransition> sti = s
								.getOutgoingTransitionsListIterator();
						boolean found = false;
						while (sti.hasNext())
						{
							...
							// of the search
							if (t.getEvent() != null
									&& t.getEvent().equals(plst.getEvent()))
							{
								found = true;
								break;
							}
	...
	/**
	 * Computes the accessible product of the two automata a and b for use with
	 * the supremal controllable sublanguage. [?This is made as a special an
	 * extra flag is set in the resulting automaton?]
	 * 
	 * @param a
	 *            an automaton
	 * @param b
