...
public class InfimalObservable implements Operation
{
	...
	public Object[] perform(Object[] inputs)
	{
		...
		// plant.
		boolean isSublanguage = (Boolean)OperationManager
		...
		FSAModel temp, temp2;
		FSAModel lBar = (FSAModel)OperationManager
		...
		FSAModel kleeneClosure = (FSAModel)OperationManager
				.instance().getOperation("kleeneclosure")
				.perform(new Object[] { plant.getEventSet() })[0];
		warnings.addAll(OperationManager
				.instance().getOperation("kleeneclosure").getWarnings());

		SupervisoryEventSet unobservableEvents = ModelManager
				...
				.hasNext();)
		{
			...
			temp = (FSAModel)OperationManager
			...
			invproj = (FSAModel)OperationManager
					.instance().getOperation("union").perform(new Object[] {
							invproj, temp })[0];
			warnings.addAll(OperationManager
					.instance().getOperation("union").getWarnings());
		}

		temp = (FSAModel)OperationManager
				.instance().getOperation("+closure")
				.perform(new Object[] { plant.getEventSet() })[0];
		warnings.addAll(OperationManager
				.instance().getOperation("+closure").getWarnings());
		...
		temp = (FSAModel)OperationManager
				.instance().getOperation("concatenate").perform(new Object[] {
						temp, kleeneClosure })[0];
		warnings.addAll(OperationManager
				.instance().getOperation("concatenate").getWarnings());
		...
		FSAModel infO = (FSAModel)OperationManager
				.instance().getOperation("setminus").perform(new Object[] {
						plant, temp })[0];
		warnings.addAll(OperationManager
				.instance().getOperation("setminus").getWarnings());
		...
		infO = (FSAModel)OperationManager
				.instance().getFilterOperation("trim")
				.filter(new Object[] { infO })[0];
		warnings.addAll(OperationManager
	...
	private FSAModel eventToModel(SupervisoryEvent e)
	{
		...
		model.add(marked);
		model.add(model.assembleTransition(initial.getId(), marked.getId(), e
				.getId()));
		return model;
	}
