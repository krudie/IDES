...
public class DuplicationToolbox
{
	 ...
	 * @param model2
	 *            The model the states are being copied from.
	 * @param states
	 *            Map to keep track of original states in model2 and their
	 *            counterparts in model1.
	 * @param copyId
	 *            When <code>true</code>, the id of each original state will be
	 *            copied to the new state
	 */
	protected static void copyAllStates(FSAModel model1, FSAModel model2,
			Map<FSAState, FSAState> states, boolean copyId)
	{
	...
	/**
	 * Produces a duplication in model1 of the original state. If a duplication
	 * has already been made, an entry will exist in the mapping and the
	 * previously duplicated state based on the same original event will be
	 * fetched. If the duplication has not already been made, a new state will
	 * be assembled, duplicated, then added to model1 and the mapping.
	 * Duplication consists of the same name, initial and marked status, and
	 * optionally id based on value of copyId as the original state.
	 * 
	 * @param model1
	 ...
	 * @param events
	 *            Contains a mapping of the event symbol to an event
	 * @param copyId
