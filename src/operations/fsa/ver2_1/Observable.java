...
public class Observable implements Operation
{
	 ...
	 * Christos G. Cassandras and Stephane Lafortune (2nd Edition) p. 188
	 * Polynomial Test for Observability (available as a pdf from the Queen's
	 * library website as of summer 2010). If the automaton reaches the marked
	 * state "dead" then the sublanguage is not observable with respect to the
	 * plant, and the method immediately returns false (i.e. construction of the
	 * automaton is terminated).
	 * 
	 * @param sublanguage
	 *            The determinisitic automaton representing the language under
	 *            consideration, that is a sublanguage of the plant.
	 * @param plant
	 *            The the deterministic automaton representing the plant or
	 *            superlanguage that observability of the sublanguage is being
	 *            determined with respect to.
	 * @param obsTest
	 *            An empty model that will contain the result automaton.
	 * @return <code> true </code> if the sublanguage is observable with respect
	 *         to the plant, <code>false</code> otherwise.
	 */
	/*
	 * This method was originally written to, and is is easily modified to
	 * return the created automaton. Simply set the return type to void, remove
	 * the return false statement when it reaches the stopping condition and
	 * change the return true to return obsTest at the end. The input/output
	 * information needs to be changed above in order to display this automaton.
	 */
			...
			FSAModel obsTest)
	{
		...
		while (!searchList.isEmpty())
		{
			...
			for (Iterator<FSATransition> i = sourceTriple[1]
					.getOutgoingTransitionsListIterator(); i.hasNext();)
			{
				currentEvents.add(i.next().getEvent());
			}
			...
			for (SupervisoryEvent event : currentEvents)
			{
				...
				stateEventMap.clear();
				transitionTriple = getOutgoingTransitionTriple(sourceTriple,
						event);
				...
				if (isStoppingCondition(transitionTriple, event))
				{
					...
					FSATransition toDead = obsTest.assembleTransition(source
							.getId(), dead.getId(), toDeadEvent.getId());
					obsTest.add(toDead);
				...
				else
				{
					if (event.isObservable())
					{
						if (transitionTriple[0] != null
								&& transitionTriple[1] != null
								&& transitionTriple[2] != null)
						{
					...
					else
					{
						if (transitionTriple[0] != null)
						{
							tempTransitionTriple = new FSATransition[] {
									transitionTriple[0], null, null };
							targetTriple = next(sourceTriple,
									tempTransitionTriple);
							stateEventMap.put(targetTriple,
									tempTransitionTriple);
						}
						if (transitionTriple[1] != null
								&& transitionTriple[2] != null)
						{
							tempTransitionTriple = new FSATransition[] { null,
									transitionTriple[1], transitionTriple[2] };
							targetTriple = next(sourceTriple,
									tempTransitionTriple);
							stateEventMap.put(targetTriple,
									tempTransitionTriple);
					...
					{
						for (Iterator<FSAState[]> i = stateEventMap
								.keySet().iterator(); i.hasNext();)
						{
							...
							if (stateid < 0)
							{
								...
								makeStateFromTriple(targetTriple,
										target,
										id,
										false);
							...
							FSATransition transition = obsTest
									.assembleTransition(source.getId(), target
											.getId(), newEvent.getId());
	...
	/**
	 * Private method to determine whether the stopping condition has been
	 * reached for a given set of transitions and event.
	 * 
	 * @param transitionTriple
	 *            An array containing the transitions from the source state
	 *            labelled with the event under consideration. If no such
	 *            transition exists, the array entry should be <code>null</code>
	 *            .
	 * @param event
	...
	/**
	 * A private method to identify the outgoing transitions labelled with a
	 * given event from a state triple. Since the automata are all
	 * deterministic, there is either only one or no such transition.
	 * 
	 * @param stateTriple
	 *            The state triple under consideration.
	 * @param event
	 *            The event under consideration.
	 * @return An array containing the transitions from the source states
	 *         labelled with the given event. If no such transitions exist, the
			...
			FSAState[] stateTriple, SupervisoryEvent event)
	{
		...
		{
			isDefined = false;
			Iterator<FSATransition> oti = stateTriple[i]
	 ...
	 * automaton. Determines the next state triple based on the transition
	 * triple.
	 * 
	 * @param stateTriple
	 *            The source state triple under consideration.
	 * @param transitionTriple
	 *            The outgoing transitions corresponding to the source states.
	 * @return The successor state triple.
	 */
	private static FSAState[] next(FSAState[] stateTriple,
			FSATransition[] transitionTriple)
	{
	 ...
	 * 
	 * @param s
	 *            the array of 3 states
	 * @param newState
	 *            a pre-assembled state that will be modified into the new state
	 * @param id
	 *            the id given to the new state
	 * @param initial
	 *            when <code>true</code> the new state will be an initial state
	 */
	 ...
	 * 
	 * @param s
	 *            the state triple to set a new id for
	 * @param n
	 *            the new id
	 */
	 ...
	 * 
	 * @param sll
	 *            the state triple to compile a string from
