...
/**
 * A dialog box with a label, a progress bar and a cancel button.
 * It implements the {@link java.lang.Runnable} interface. The code which
 * performs the operations whose progress has to be displayed can
 * reside in the {@link #run()} method. Thus the whole dialog box
 * can be run as a thread. The progress bar has to be updated manually.
 * If the user chooses to cancel the process, the {@link #interrupt()}
 * method will be called and your code has to react appropriately.
 * @see #run()
 * @see #progressBar
 * @see #interrupt()
 *
...
public abstract class InterruptableProgressDialog extends EscapeDialog
	 ...
	 * <p>Its position has to be update manually.
	 * @see #run()
	 */
	 ...
	 * so that it can handle interruption by the user. 
	 ...
	 * <p>Place here the code that will halt the progress of your process.
	 * @see #run()
	 */
	 ...
	 * <p>Place here the code which performs the activities whose progress is displayed.
	 * <p>You have to manually update the progress bar.
	 * @see #progressBar
	 * @see #interrupt()
	 */
