...
public class Node extends GraphObject {
     ...
     */
    private Node(GraphingPlatform gp, GraphModel gm, int x, int y, int r,
            int attributes, float arrow_x, float arrow_y, GlyphLabel glyph_label) {
     ...
     */
    public Node(GraphingPlatform gp, GraphModel gm, int x, int y, int r,
            int attributes, float arrow_x, float arrow_y, String glyph_string) {
        super(gp, gm, attributes);
        this.x = x;
        this.y = y;
        constructNode(r, arrow_x, arrow_y);
    ...
    private void initializeLabels(String glyph_string) {
        glyph_label = new GlyphLabel(gp, this, glyph_string, origin(),
                Label.CENTER);
    }
    ...
    public void draw(Drawer drawer) {
        ...
        if (isStartState()) {
            ...
            drawer.drawLine(arrowhead.xcoords[ArrowHead.NOCK],
                    arrowhead.ycoords[ArrowHead.NOCK],
                    arrowhead.xcoords[ArrowHead.NOCK]
                            - (int) Math.round(ArrowHead.HEAD_LENGTH
                                    * start_arrow_direction.x),
                    arrowhead.ycoords[ArrowHead.NOCK]
                            - (int) Math.round(ArrowHead.HEAD_LENGTH
        ...
        if (origional_configuration != null) {
            if (!exclusive) {
                Point displacement = new Point(mouse.x
                        - origional_configuration.movement_origin.x, mouse.y
                        - origional_configuration.movement_origin.y);
                this.x = origional_configuration.origin.x + displacement.x;
                this.y = origional_configuration.origin.y + displacement.y;

                snapToGrid();

                arrowhead.update(start_arrow_direction,
                        origional_configuration.arrow_tip.x + this.x
                                - origional_configuration.origin.x,
            ...
            for (int i = 0; i < edge_group_list.size(); i++) {
                edge_group = (EdgeGroup) edge_group_list.elementAt(i);
                if (!exclusive || !edge_group.exclusive) {
                    edge_group.updateNodeMovement(this,
    ...
    public boolean isBoundBy(Box area) {
        if ((x - r > area.x1() && x + r < area.x2())
                && (y - r > area.y1() && y + r < area.y2())) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean isLocatedStartArrow(Point mouse) {
        if (isStartState()) {
            return arrowhead.isLocated(mouse.x, mouse.y);
        } else {
            return false;
        }
    }
    ...
    public EdgeGroup join(Edge new_edge, Node new_neighbour) {
        ...
        for (int i = 0; i < edge_group_list.size(); i++) {
            ...
            if (new_neighbour == this) {
                if (edge_group.hasNodes(this, this)) {
                    found_match = true;
                    break;
                }
            } else {
                if (edge_group.hasNode(new_neighbour)) {
                    found_match = true;
                    break;
                }
            }
    ...
    public Edge getEdge(int machine_code) {
        Edge e = null;
        for (int i = 0; i < edge_group_list.size(); i++) {
            e = ((EdgeGroup) edge_group_list.elementAt(i)).getEdge(
                    machine_code, this);
            if (e != null) {
                break;
            }
