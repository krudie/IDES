...
public class GlyphLabel extends Label {
     ...
     */
    private static final Font FONT = new java.awt.Font("Helvetica",
     ...
     */
    private Vector glyph_vector_vector = null, glyph_shape_vector = null;
     ...
     */
    public GlyphLabel(GraphingPlatform gp, GraphObject parent,
            String string_representation, Point anchor, int anchor_type) {
        super(gp, parent, string_representation, anchor, anchor_type,
                MINIMUM_RADIUS);
    }
    ...
    public GlyphLabel(GraphingPlatform gp, GraphObject parent, GlyphLabel source) {
        super(gp, parent, source.string_representation, source.anchor,
                source.anchor_type, source.rendered_radius);
    ...
    public void render() {
        if (string_representation.length() > 0) {
            try {
                ...
                gp.shell.setCursor(ResourceManager
                        .getCursor(ResourceManager.WAIT_CURSOR));
                ...
                rendered_radius = Math.round(Geometric.magnitude(
                        glyph_bounds.height, glyph_bounds.width) / 2)
                ...
                gp.shell.setCursor(ResourceManager
                        .getCursor(ResourceManager.ARROW_CURSOR));
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private void renderAtCenter() {
        ...
        StringTokenizer s = new StringTokenizer(glyph_string,
        ...
        while (s.hasMoreTokens()) {
            ...
            glyph_vector = FONT.createGlyphVector(FONT_RENDER_CONTEXT,
                    glyph_string);
            glyph_vector_vector.addElement(glyph_vector);
            glyph_bounds = glyph_vector.getPixelBounds(glyph_vector
        ...
        for (int i = 0; i < glyph_vector_vector.size(); i++) {
            glyph_bounds = ((GlyphVector) glyph_vector_vector.elementAt(i))
                    .getPixelBounds(glyph_vector.getFontRenderContext(), 0, 0);
            glyph_shape = ((GlyphVector) glyph_vector_vector.elementAt(i))
                    .getOutline(anchor.x - glyph_bounds.x - glyph_bounds.width
                            / 2, anchor.y - glyph_bounds.y - box_height / 2 + i
        ...
        glyph_bounds = new Rectangle(anchor.x - box_width / 2, anchor.y
    ...
    private void renderAtCorner() {
        ...
        Shape glyph_shape = glyph_vector.getOutline(modified_anchor.x,
                modified_anchor.y);
        glyph_bounds = glyph_vector.getPixelBounds(glyph_vector
                .getFontRenderContext(), modified_anchor.x, modified_anchor.y);
    ...
    public void drawBox(Drawer drawer) {
        drawer.drawBoxWH(glyph_bounds.x, glyph_bounds.y, glyph_bounds.width,
                glyph_bounds.height, Drawer.SMALL_DASHED);
    }
    ...
    public void drawTether(Drawer drawer, Point anchor) {
        drawer.drawLine(anchor.x, anchor.y, glyph_bounds.x, glyph_bounds.y,
                Drawer.SMALL_DASHED);
    }
    ...
    public void setAnchor(Point anchor, int anchor_type) {
        boolean doit = true;
        if (this.anchor != null) {
            if (anchor.isSameAs(this.anchor) && anchor_type == this.anchor_type) {
                doit = false;
            }
        }
        if (doit) {
            ...
            if (glyph_bounds != null && glyph_vector_vector != null
                    && anchor_type == CENTER) {
                ...
                Shape glyph_shape = null;
                for (int i = 0; i < glyph_vector_vector.size(); i++) {
                    these_bounds = ((GlyphVector) glyph_vector_vector
                            .elementAt(i)).getPixelBounds(FONT_RENDER_CONTEXT,
                            0, 0);
                    glyph_shape = ((GlyphVector) glyph_vector_vector
                            .elementAt(i)).getOutline(anchor.x - these_bounds.x
                            - these_bounds.width / 2, anchor.y - these_bounds.y
                            - unpadded_box_height / 2 + i
                            * (GLYPH_HEIGHT + LINE_SPACE));
                    glyph_shape_vector.addElement(glyph_shape);
                }
            } else if (anchor_type == CORNER
                    && string_representation.length() > 0) {
                renderAtCorner();
            }
        }
    }
