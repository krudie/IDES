...
/**
 * Modifications by Michael Wood
 * added \\setlength{\\parindent}{0pt} to default preamble in two methods.
 */

/**
 *
 * This class allows for rendering of LaTeX documents from within Java.
 * It can be used also for the coversion of LaTeX documents into EPS files
 * (only the first page is output).
 * <p>The external software that has to be available is:
 * <ul>
 * 	<li><b>latex</b>
 * 	<li><b>dvips</b> (found in the same folder as latex)
 * 	<li><b>GhostScript</b> (preferrably version 8+)
 * </ul>
 * <p>To use the renderer:
 * <pre>
...
public class Renderer {
	...
	private class Executor extends Thread
	{
		...
		/**
		 * Executes the call as specified during the construction of this object.
		 * After the call completes, <code>done</code> is set to true and all
		 * threads waiting on this Executor are awoken. The thread
		 * can be interrupted (via {@link java.lang.Thread#interrupt()}) in case
		 * of a timeout; if this happens, the call is aborted.
		 */
		public void run()
		{
			...
			try
			{
				...
				try{
					proc.waitFor();
				}catch (InterruptedException e){ proc.destroy(); }
		...
		/**
		 * Returns the command (without parameters) wich this object will execute.
		 * This can be used in announcements in case of timeouts.
	 ...
	 * @param f	file where to save image. If the file exists, its
	 ...
	 * This method is called by all other rendering methods. The method removes
	 * all temporary files (including the file in the parameter) after finishing
	 * the rendering.
	...
	/**
	 * Renders the first page of the LaTeX document contained in a file
	 * into an uncropped image.
	 * @param latexFile	file containing the LaTeX document
	 * @param dpi	the resolution to be used for the redering (in dots per inch)
	...
	protected BufferedImage latex2PNG(File latexFile, int dpi) throws IOException, LatexRenderException
	{
		...
		BufferedImage im=new BufferedImage(tempIm.getWidth(null),tempIm.getHeight(null),BufferedImage.TYPE_BYTE_GRAY);
	 ...
	 * <p>For example, if the parameter is the file "c:\temp\ides0013.tex",
	 * then the files "c:\temp\ides0013.*" will be deleted.
	 * @param latexFile	file whose name and location will be used to determine
	...
	private void removeTempFiles(File latexFile)
	{
		...
		for(int i=0;i<files.length;++i)
		{
			if(files[i].lastIndexOf('.')<0)
				continue;
			if(files[i].substring(0,files[i].lastIndexOf('.')).equals
			(latexFile.getName().substring(0,latexFile.getName().lastIndexOf('.'))))
			{
				try
				{
	...
	/**
	 * Execute a call to an external program as per an {@link Renderer.Executor} object,
	 * using a timeout of {@link #TIMEOUT} milliseconds.
	 * @param ex	the {@link Renderer.Executor} with the call that has to be executed 
	 * @throws LatexRenderException	if the call timed out or if the call didn't succeed
	...
	/**
	 * Finds the bounding box of an image with a single band (grayscale). 
	 * The bounding box is defined as the smallest rectangle
	 * such that all pixels outside the rectangle are white.
	 ...
	 * @param doc	LaTeX document
	 * @param target	file where to save the EPS. If the file exists, its
	 *		 			content will be overwritten.
	 * @throws IOException	if there was a problem with the IO
	 * @throws LatexRenderException	if there was a problem while rendering the LaTeX code
	 ...
	 * into EPS. The method removes all temporary files (including the file in the
	...
	protected void tempLatex2EPS(File latexFile,File target) throws IOException, LatexRenderException
	{
		...
		try
		{
			im=latex2PNG(latexFile,72); //bounding boxes are computed using 72dpi metrics
			Rectangle r=findBoundingBox(im.getData());
			r.height=r.height+1;
			r.width=r.width+1;
			r.y=im.getHeight()-r.y-r.height; //y=0 is at the bottom of the page in EPS
			fixBoundingBox(r,new File(latexFile.getCanonicalPath().substring(0,latexFile.getCanonicalPath().lastIndexOf('.'))+".ps"),target);
		} finally //remove temporary files
		{
			removeTempFiles(latexFile);
		}
	...
	/**
	 * Fixes the <code>BoundingBox</code> comment in EPS files so it reflects the true
	 * boundary of the graphic. Some tools (notably <code>dvips</code>) may not always
	 * generate a correct <code>BoundingBox</code> comment.
	...
	protected void fixBoundingBox(Rectangle boundary, File source, File target) throws IOException
	{
		...
		while((b=in.read())>=0) //loop to find "%%BoundingBox:"
		{
			...
			if(b==10) //if new line
			{
				...
				int[] buffer=null;
				while(!exitLoop) //loop is repeated if new line is encountered
				{
					exitLoop=true;
					buffer=new int[BBSTRING_LENGTH];
					for(int i=0;i<BBSTRING_LENGTH;++i) //read string to match
					{
						buffer[i]=in.read();
						...
						else if(buffer[i]<0) //EOF was encountered
							break;
					}
				}
				boolean matched=true;
				for(int i=0;i<BBSTRING_LENGTH;++i)	//match buffer & string
				...
				else //string was matched
				{
					while((b=in.read())!=10) //fast forward to end of line
						;
					out.write(("%%BoundingBox: "+boundary.x+" "+boundary.y+
		...
		while((b=in.read())>=0) //just copy rest of file
			out.write(b);
		in.close();
		out.close();
	}
	...
	{
		Renderer t=Renderer.getRenderer(new File("c:\\program files\\texmf\\miktex\\bin"),new File("C:\\Program Files\\gs\\gs8.14\\bin"));
		Image i=t.renderString("$|\\Sigma|$");
		t.renderStringToFile("$|\\Sigma|$",new File("c:\\!work\\foo.png"));
		t.latex2EPS(new File("c:\\!work\\graph1a.tex"),new File("c:\\!work\\graph1b.eps"));
		javax.swing.JFrame win=new javax.swing.JFrame();
		javax.swing.ImageIcon icon=new javax.swing.ImageIcon(i);
		javax.swing.JButton p=new javax.swing.JButton(icon);
