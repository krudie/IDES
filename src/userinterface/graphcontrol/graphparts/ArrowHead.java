...
public class ArrowHead
{
	...
	/**
     * The co-ordinates of the ArrowHead.
     * tip,tang,nock,tip (clockwise).
     */
	public int[] xcoords = new int[4],
				 ycoords = new int[4];
     ...
     */
	public ArrowHead() { visible = false; }
	
    /**
     * Construct the ArrowHead.
     *
     * @param	d		A unit direction from nock to tip.
     * @param	tip_x	The x co-ordinate of the tip.
     * @param	tip_y	The y co-ordinate of the tip.
     */
	public ArrowHead(UnitVector d, int tip_x, int tip_y) { update(d,tip_x,tip_y); }
	
    /**
     * Construct the ArrowHead.
     *
     * @param	d		A unit direction from nock to tip.
     * @param	tip		The tip point.
     */
	public ArrowHead(UnitVector d, Point tip) { update(d,tip); }
	...
	{
		if (visible) { drawer.drawPolygon(xcoords, ycoords, 4); } 
	}
     ...
     * Update the ArrowHead.
     *
     * @param	d	A UnitVector from nock to tip.
     * @param	tip	A Point representing the tip.
     */
	...
	public boolean isLocated(Point mouse, boolean padded)
	{
		...
		if (padded) { test_radius = 2*ArrowHead.HEAD_LENGTH; }
		
		if (visible)
		{
			// this is a simple circle test
			if (Math.pow(xcoords[ArrowHead.NOCK]-mouse.x,2) + Math.pow(ycoords[ArrowHead.NOCK]-mouse.y,2) < Math.pow(test_radius,2)) 
			{ return true; }
		}
		return false;
	}	
