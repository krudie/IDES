...
public class Curve {
     ...
     */
    private float tail_anchor_angle = 0, tail_ctrl_angle = 0,
            tail_ctrl_radius = 0, tail_ctrl_mag_factor = 0,
            tail_ctrl_ratio = 0, head_ctrl_ratio = 0, head_ctrl_mag_factor = 0,
            head_ctrl_radius = 0, head_ctrl_angle = 0, head_anchor_angle = 0;
     ...
     */
    public Curve(Node tail_node, Node head_node, float x1, float y1,
            float ctrlx1, float ctrly1, float ctrlx2, float ctrly2, float x2,
            float y2, UnitVector self_loop_direction) {
        this.self_loop_direction = self_loop_direction;
        this.tail_node = tail_node;
        this.head_node = head_node;
        curve = new CubicCurve2Dex(x1, y1, ctrlx1, ctrly1, ctrlx2, ctrly2, x2,
                y2);
        arrowhead = new ArrowHead();
        updateArrowhead();
    ...
    public Curve newClone(Node tail_node, Node head_node) {
        ...
        return new Curve(tail_node, head_node, curve.x1, curve.y1,
                curve.ctrlx1, curve.ctrly1, curve.ctrlx2, curve.ctrly2,
                curve.x2, curve.y2, cloned_direction);
    }
    ...
    protected float intersectWithCircle(int radius, boolean startPoint) {
        ...
        float distance = (float) Math.sqrt((x - b.x) * (x - b.x) + (y - b.y)
                * (y - b.y))
                - radius;
        while (Math.round(distance) > 0) {
            ...
            distance = (float) (Math.sqrt((x - b.x) * (x - b.x) + (y - b.y)
                    * (y - b.y)) - radius);
        }
        if (t < 0)
            t = 0;
    ...
    protected float travelEdge(float start, float distance, boolean startToEnd) {
        ...
        Point b = calculateBezierPoint(t);
        float d = distance
                - (float) Math.sqrt((s.x - b.x) * (s.x - b.x) + (s.y - b.y)
                        * (s.y - b.y));
        while (Math.round(d) > 0) {
            if (startToEnd)
                ...
                t -= d / curve.maxLength();
            b = calculateBezierPoint(t);
            d = (float) (distance - Math.sqrt((s.x - b.x) * (s.x - b.x)
                    + (s.y - b.y) * (s.y - b.y)));
        }
        if (t < 0)
            t = 0;
     ...
     */
    public void moveTailAnchor(Configuration origional_configuration,
            Point mouse) {
        // update the anchor
        ...
        if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL) {
            ...
            if (directions.x < 0) {
                ...
                float mag = Geometric.distance(
                        origional_configuration.tail_anchor,
    ...
    public void moveTailCtrl(Configuration origional_configuration, Point mouse) {
        ...
        if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL) {
            ...
            if (directions.x < 0) {
                ...
                Point new_anchor = new Point(curve.x1 + mag * d.x, curve.y1
                        + mag * d.y);
    ...
    public void moveHeadCtrl(Configuration origional_configuration, Point mouse) {
        ...
        if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL) {
            ...
            if (directions.x < 0) {
                ...
                Point new_anchor = new Point(curve.x2 + mag * d.x, curve.y2
                        + mag * d.y);
     ...
     *            The current mouse position.
     */
    public void moveHeadAnchor(Configuration origional_configuration,
            Point mouse) {
        // pad the circle
        int padded_radius = head_node.r() + ArrowHead.SHORT_HEAD_LENGTH;
        ...
        if ((origional_configuration.state_mask & SWT.CTRL) != SWT.CTRL) {
            ...
            if (directions.x < 0) {
                ...
                // perpendicular of the radial arm.
                float mag = Geometric.distance(
                        origional_configuration.head_anchor,
    ...
    public void recalculateSelfLoop() {
        Point p1 = self_loop_direction.newPoint(tail_node.r(), tail_node
                .origin(), 25);
        Point p2 = self_loop_direction.newPoint(tail_node.r() + 40, tail_node
                .origin(), 32);
        Point p3 = self_loop_direction.newPoint(tail_node.r() + 35, tail_node
                .origin(), -28);
        Point p4 = self_loop_direction.newPoint(tail_node.r()
                + ArrowHead.SHORT_HEAD_LENGTH, tail_node.origin(), -25);
        Point p5 = self_loop_direction.newPoint(tail_node.r(), tail_node
                .origin(), -25);
        setTailAnchor(p1);
        setTailCtrl(p2);
        setHeadCtrl(p3);
    ...
    public void moveSelfLoop(Configuration origional_configuration, Point mouse) {
        ...
        self_loop_direction
                .rotateByDegrees(origional_configuration.selection_error_fix);
        recalculateSelfLoop();
    }
    ...
    private void updateArrowhead() {
        ...
        Point tip = new Point(head_node.origin().x + head_node.r() * d.x,
                head_node.origin().y + head_node.r() * d.y);
        d.reverse();
        ...
        tip_to_nock.reverse();
        curve.x2 = arrowhead.xcoords[ArrowHead.TIP]
                + ArrowHead.SHORT_HEAD_LENGTH * tip_to_nock.x;
        curve.y2 = arrowhead.ycoords[ArrowHead.TIP]
                + ArrowHead.SHORT_HEAD_LENGTH * tip_to_nock.y;
    }
    ...
    public void initiateNodeMovement() {
        ...
        // useful directions
        UnitVector tail_to_head = new UnitVector(tail_node.origin(), head_node
                .origin());
        UnitVector head_to_tail = tail_to_head.newReversed();

        // tail config
        Point on_tail_bisector = new Point(tail_node.origin(), tail_to_head,
                100);
        tail_anchor_angle = -Geometric.calculateRadians(on_tail_bisector,
                tail_node.origin(), tailAnchor());
        tail_ctrl_angle = -Geometric.calculateRadians(on_tail_bisector,
                tail_node.origin(), tailCtrl());
        ...
        float tail_ctrl_flat_mag = Geometric.dotProduct(tail_ctrl_vector,
                tail_to_head)
                / (Geometric.magnitude(tail_ctrl_vector) * (float) Math
                        .cos(tail_ctrl_angle));
        tail_ctrl_mag_factor = Geometric.floatBounds(tail_ctrl_flat_mag
                / anchors_mag);
        // find the ratio of flat_mag to radial_mag for this ctrl
        tail_ctrl_ratio = Geometric.floatBounds(tail_ctrl_flat_mag
                / tail_ctrl_radius);

        // head config
        Point on_head_bisector = new Point(head_node.origin(), head_to_tail,
                100);
        head_anchor_angle = -Geometric.calculateRadians(on_head_bisector,
                head_node.origin(), headAnchor());
        head_ctrl_angle = -Geometric.calculateRadians(on_head_bisector,
                head_node.origin(), headCtrl());
        head_ctrl_radius = Geometric.distance(head_node.origin(), headCtrl());

        // find the fraction of the distance along the bisector direction that
        // this ctrl traverses
        Point head_ctrl_vector = headCtrl().minus(headAnchor());
        float head_ctrl_flat_mag = Geometric.dotProduct(head_ctrl_vector,
                head_to_tail)
                / (Geometric.magnitude(head_ctrl_vector) * (float) Math
                        .cos(head_ctrl_angle));
        head_ctrl_mag_factor = Geometric.floatBounds(head_ctrl_flat_mag
                / anchors_mag);
        // find the ratio of flat_mag to radial_mag for this ctrl
        head_ctrl_ratio = Geometric.floatBounds(head_ctrl_flat_mag
                / head_ctrl_radius);
    }
     ...
     */
    public void updateNodeMovement(Configuration origional_configuration,
            Point mouse, Node fixed_circle, Node moving_circle) {
        // useful directions
        UnitVector tail_to_head = new UnitVector(tail_node.origin(), head_node
                .origin());
        UnitVector head_to_tail = tail_to_head.newReversed();
        ...
        UnitVector tail_anchor_direction = tail_to_head
                .newRotatedByRadians(tail_anchor_angle);
        UnitVector tail_ctrl_direction = tail_to_head
                .newRotatedByRadians(tail_ctrl_angle);
        UnitVector head_ctrl_direction = head_to_tail
                .newRotatedByRadians(head_ctrl_angle);
        UnitVector head_anchor_direction = head_to_tail
                .newRotatedByRadians(head_anchor_angle);

        // calculate new anchor positions
        setTailAnchor(tail_node.origin().newPoint(tail_anchor_direction,
                tail_node.r()));
        ...
        float new_tail_ctrl_radius = Geometric
                .floatBounds((new_anchors_mag * tail_ctrl_mag_factor)
                        / tail_ctrl_ratio);
        float new_head_ctrl_radius = Geometric
                .floatBounds((new_anchors_mag * head_ctrl_mag_factor)
                        / head_ctrl_ratio);

        // this gives bad behaviour if a ctrl radius is greater than the
        // distance between the two nodes.
        float node_distance = Geometric.distance(tail_node.origin(), head_node
                .origin())
                - tail_node.r() - head_node.r();
        if (node_distance < 0) {
            node_distance = 0;
        }
        if (new_tail_ctrl_radius > node_distance) {
            new_tail_ctrl_radius = node_distance;
        }
        if (new_head_ctrl_radius > node_distance) {
            new_head_ctrl_radius = node_distance;
        }
        ...
        if (Geometric.distance(tail_node.origin(), tailCtrl()) < tail_node.r()
                + 2 * ANCHOR_RADIUS) {
            ctrl_adjust = new Line(tail_node.origin(), tailCtrl());
            curve.ctrlx1 = tail_node.origin().x
                    + (tail_node.r() + 2 * ANCHOR_RADIUS) * ctrl_adjust.d.x;
            curve.ctrly1 = tail_node.origin().y
                    + (tail_node.r() + 2 * ANCHOR_RADIUS) * ctrl_adjust.d.y;
        }
        if (Geometric.distance(head_node.origin(), headCtrl()) < head_node.r()
                + 2 * ANCHOR_RADIUS + ArrowHead.SHORT_HEAD_LENGTH) {
    ...
    public void calculateCurve(float rise, float angle) {
        if (SELECTED_METHOD == 1) {
            ...
            UnitVector d = new UnitVector(tail_node.origin(), head_node
                    .origin());
            ...
            int padded_radius = head_node.r() + ArrowHead.SHORT_HEAD_LENGTH;
            setTailAnchor(new Point((int) Math.round(tail_node.x()
                    + tail_node.r() * d1.x), (int) Math.round(tail_node.y()
                    + tail_node.r() * d1.y)));
            setHeadAnchor(new Point((int) Math.round(head_node.x()
                    - padded_radius * d2.x), (int) Math.round(head_node.y()
                    - padded_radius * d2.y)));
            ...
            setTailCtrl(new Point((int) Math.round(tailAnchor().x + run * d.x
                    - rise * dperp.x), (int) Math.round(tailAnchor().y + run
                    * d.y - rise * dperp.y)));
            setHeadCtrl(new Point((int) Math.round(headAnchor().x - run * d.x
                    - rise * dperp.x), (int) Math.round(headAnchor().y - run
                    * d.y - rise * dperp.y)));
    ...
    public Point tailCtrl() {
        return new Point((int) Math.round(curve.ctrlx1), (int) Math
                .round(curve.ctrly1));
    }
    ...
    public Point headCtrl() {
        return new Point((int) Math.round(curve.ctrlx2), (int) Math
                .round(curve.ctrly2));
    }
    ...
    public float headAnchorAngle() {
        UnitVector head_to_tail = new UnitVector(head_node.origin(), tail_node
                .origin());
        Point on_head_bisector = new Point(head_node.origin(), head_to_tail,
                100);
        return -Geometric.calculateRadians(on_head_bisector,
                head_node.origin(), headAnchor());
    }
     ...
     * 
     * @param out
     *            An initialized PrintWriter for outputing the data to file.
     */
    ...
    public Point calculateBezierPoint(float t) {
        float x = (1 - t) * (1 - t) * (1 - t) * curve.x1 + 3 * t * (1 - t)
                * (1 - t) * curve.ctrlx1 + 3 * t * t * (1 - t) * curve.ctrlx2
                + t * t * t * curve.x2;
        float y = (1 - t) * (1 - t) * (1 - t) * curve.y1 + 3 * t * (1 - t)
                * (1 - t) * curve.ctrly1 + 3 * t * t * (1 - t) * curve.ctrly2
                + t * t * t * curve.y2;
    ...
    public int isLocatedSelfLoop(Point mouse, boolean padded) {
        ...
        if (padded) {
            test_radius = 2 * ANCHOR_RADIUS;
        }
        if (selfLoopAnchor().isInsideCircle(test_radius, mouse)) {
            return Edge.R_LOOP;
        } else {
            return Edge.R_NONE;
        }
    ...
    public int isLocatedAnchors(Point mouse, boolean padded) {
        int test_radius = ANCHOR_RADIUS;
        if (padded) {
            test_radius = 4 * ANCHOR_RADIUS;
        }
        if (tailAnchor().isInsideCircle(test_radius, mouse)) {
            return Edge.R_TAIL_ANCHOR;
        } else if (tailCtrl().isInsideCircle(test_radius, mouse)) {
            return Edge.R_TAIL_CTRL;
        } else if (headCtrl().isInsideCircle(test_radius, mouse)) {
            return Edge.R_HEAD_CTRL;
        } else if (headAnchor().isInsideCircle(test_radius, mouse)) {
            return Edge.R_HEAD_ANCHOR;
        } else {
            return Edge.R_NONE;
        }
    }
    ...
    public int isLocatedArrowhead(Point mouse, boolean padded) {
        ...
        if (arrowhead.isLocated(mouse)) {
            return Edge.R_ARROWHEAD;
        } else {
            return Edge.R_NONE;
        }
    }
    ...
    private void calculateCurve1(float rise_factor, float angle) {
        ...
        // rise
        float mag = (float) Math.sqrt(Math.pow(p1.x - p4.x, 2)
                + Math.pow(p1.y - p4.y, 2)) / 3; // one third of the distance
        ...
        Point p2 = new Point((int) Math.round(tail_node.x() + mag * d.x - rise
                * dperp.x), (int) Math.round(tail_node.y() + mag * d.y - rise
                * dperp.y));
        Point p3 = new Point((int) Math.round(head_node.x() - mag * d.x - rise
                * dperp.x), (int) Math.round(head_node.y() - mag * d.y - rise
                * dperp.y));
