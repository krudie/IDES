...
/**
 * This subclass of Graphics2D allows leaf objects Nodes and Edges and
 * ArrowHeads to draw themselves, while being ignorant of color, stroke and
 * graphics classes. Their options in this regard are limited to the constants
 * provided to them by their GraphModel. This also keeps AWT out of most of the
 * classes.
 * 
 * @author Michael Wood
 */
public class Drawer {
    ...
    public Drawer(GraphingPlatform gp, Graphics2D g2d, float scale) {
        ...
        dashed_stroke[0] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 0.5f, 0, 1 }, 1);
        dashed_stroke[1] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 1, 0, 1 }, 1);
        dashed_stroke[2] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 2, 0, 1 }, 1);
        dashed_stroke[3] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 4, 0, 1 }, 1);
        dashed_stroke[4] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 8, 0, 1 }, 1);
        dashed_stroke[5] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 1, new float[] { 16, 0, 1 }, 1);
        small_dashed_stroke[0] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        0.5f, 0, 1 }, 1);
        small_dashed_stroke[1] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        1, 0, 1 }, 1);
        small_dashed_stroke[2] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        2, 0, 1 }, 1);
        small_dashed_stroke[3] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        4, 0, 1 }, 1);
        small_dashed_stroke[4] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        8, 0, 1 }, 1);
        small_dashed_stroke[5] = new BasicStroke(stroke_width / 2,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        16, 0, 1 }, 1);
        tiny_dashed_stroke[0] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        0.5f, 0, 1 }, 1);
        tiny_dashed_stroke[1] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        1, 0, 1 }, 1);
        tiny_dashed_stroke[2] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        2, 0, 1 }, 1);
        tiny_dashed_stroke[3] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        4, 0, 1 }, 1);
        tiny_dashed_stroke[4] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        8, 0, 1 }, 1);
        tiny_dashed_stroke[5] = new BasicStroke(stroke_width / 8,
                BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1, new float[] {
                        16, 0, 1 }, 1);
     ...
     * 
     * @param color_handle
     *            An index to the GraphController's color_set from the constants
     *            in the GraphModel.
     */
    ...
    private BasicStroke dashedStroke() {
        if (scale < 1) {
            return dashed_stroke[0];
        } else if (scale == 1) {
            return dashed_stroke[1];
        } else if (scale == 2) {
            return dashed_stroke[2];
        } else if (scale == 4) {
            return dashed_stroke[3];
        } else if (scale == 8) {
            return dashed_stroke[4];
        } else {
            return dashed_stroke[5];
        }
    }

    private BasicStroke smallDashedStroke() {
        if (scale < 1) {
            return small_dashed_stroke[0];
        } else if (scale == 1) {
            return small_dashed_stroke[1];
        } else if (scale == 2) {
            return small_dashed_stroke[2];
        } else if (scale == 4) {
            return small_dashed_stroke[3];
        } else if (scale == 8) {
            return small_dashed_stroke[4];
        } else {
            return small_dashed_stroke[5];
        }
    }

    private BasicStroke tinyDashedStroke() {
        if (scale < 1) {
            return tiny_dashed_stroke[0];
        } else if (scale == 1) {
            return tiny_dashed_stroke[1];
        } else if (scale == 2) {
            return tiny_dashed_stroke[2];
        } else if (scale == 4) {
            return tiny_dashed_stroke[3];
        } else if (scale == 8) {
            return tiny_dashed_stroke[4];
        } else {
            return tiny_dashed_stroke[5];
        }
    }
    ...
    public void drawCircle(int x, int y, int r, int style) {
        if (style == SMALL_SOLID) {
            g2d.setStroke(small_solid_stroke);
        }
        if (scale == 1) {
    ...
    public void drawLine(int x1, int y1, int x2, int y2, int style) {
        ...
        } else if (style == SMALL_DASHED) {
            g2d.setStroke(smallDashedStroke());
        } else if (style == TINY_DASHED) {
            g2d.setStroke(tinyDashedStroke());
        }
        if (scale == 1) {
    ...
    public void drawBoxWH(int x, int y, int width, int height, int style) {
        ...
        if (scale == 1) {
            ...
            g2d.draw(new Rectangle2D.Float(x * scale, y * scale, width * scale,
                    height * scale));
        }
        if (style != SOLID) {
            g2d.setStroke(solid_stroke);
    ...
    public void drawCurve(CubicCurve2Dex curve, int style) {
        ...
        if (scale == 1) {
            ...
            g2d.draw(new CubicCurve2Dex(scale * curve.x1, scale * curve.y1,
                    scale * curve.ctrlx1, scale * curve.ctrly1, scale
                            * curve.ctrlx2, scale * curve.ctrly2, scale
                            * curve.x2, scale * curve.y2));
        }
        if (style == DASHED) {
            g2d.setStroke(solid_stroke);
        }
