...
public class Drawer{
     ...
     */
    public static final int SOLID = 0, DASHED = 1, SMALL_SOLID = 2, SMALL_DASHED = 3,
            TINY_DASHED = 4;
     ...
     * The scale at which drawing should be done.
     */
    private BasicStroke solid_stroke = null, dashed_stroke[] = null, small_solid_stroke = null,
            small_dashed_stroke[] = null, tiny_dashed_stroke[] = null;
    ...
    public Drawer(GraphingPlatform gp, Graphics2D g2d, float scale){
        ...
        for(int i = 0; i < 6; i++){
            dashed_stroke[i] = new BasicStroke(stroke_width, BasicStroke.CAP_BUTT,
                    BasicStroke.JOIN_ROUND, 1, new float[] {0.5f * (float) Math.pow(2, i), 0, 1}, 1);
            small_dashed_stroke[i] = new BasicStroke(stroke_width / 2, BasicStroke.CAP_BUTT,
                    BasicStroke.JOIN_ROUND, 1, new float[] {0.5f * (float) Math.pow(2, i), 0, 1}, 1);
            tiny_dashed_stroke[i] = new BasicStroke(stroke_width / 8, BasicStroke.CAP_BUTT,
                    BasicStroke.JOIN_ROUND, 1, new float[] {0.5f * (float) Math.pow(2, i), 0, 1}, 1);
        }
    ...
    public void drawCircle(int x, int y, int r, int style){
        ...
        if(scale == 1){
            ...
            g2d.draw(new Ellipse2D.Float((x - r) * scale, (y - r) * scale, 2 * r * scale, 2 * r
                    * scale));
        }
        if(style == SMALL_SOLID){
            g2d.setStroke(solid_stroke);
    ...
    public void drawBoxWH(int x, int y, int width, int height, int style){
        ...
        if(scale == 1){
            ...
            g2d.draw(new Rectangle2D.Float(x * scale, y * scale, width * scale, height * scale));
        }
        if(style != SOLID){
            g2d.setStroke(solid_stroke);
        }
    ...
    public void drawCurve(CubicCurve2Dex curve, int style){
        ...
        if(scale == 1){
            ...
            g2d.draw(new CubicCurve2Dex(scale * curve.x1, scale * curve.y1, scale * curve.ctrlx1,
                    scale * curve.ctrly1, scale * curve.ctrlx2, scale * curve.ctrly2, scale
    ...
    public void draw(BufferedImage buffered_image, int x, int y){
        ...
        else if(Math.round(buffered_image.getWidth() * scale) != 0
                && Math.round(buffered_image.getHeight() * scale) != 0){
            Image image = buffered_image.getScaledInstance(Math.round(buffered_image.getWidth()
                    * scale), Math.round(buffered_image.getHeight() * scale), 0);
