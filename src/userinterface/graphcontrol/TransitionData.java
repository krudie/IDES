...
/**
 * This class handles the creation and management of everything inside the Graph
 * Specification tab. Specifically it handles the managment of all transition
 * data, their names, descriptions, controllability, etc. In the GraphModel,
 * individual Edges maintin pointers to various entries in this class. It is
 * safe to change and destroy information here, because the edges always check
 * if the entries still exist before trying to access any of their data.
 * 
 * @author Michael Wood
 */
public class TransitionData {
     ...
     */
    public static final int SPEC_NAME = 0, SPEC_LATEX = 1, SPEC_SYMBOL = 2,
            SPEC_CONTROLLABLE = 3, SPEC_OBSERVABLE = 4, SPEC_MACHINE_CODE = 5,
            SPEC_DESCRIPTION = 6;

    /**
     * String identifiers for the various columns, indicies are relative to the
     * table columns.
     */
    public final String[] spec_strings = { "NAME", "LATEX", "SYMBOL",
            "CONTROLLABLE", "OBSERVABLE", "MACHINE_CODE", "DESCRIPTION" };
     ...
     * The table columns.
     */
    private TableColumn col_name = null, col_latex = null, col_symbol = null,
            col_controllable = null, col_observable = null,
            col_machine_code = null, col_description = null;
     ...
     */
    private Text txt_name = null, txt_symbol = null, txt_machine_code = null,
            txt_description = null, txt_latex = null, txt_trace = null;

    /**
     * The combo boxes in the editable region
     */
    private Combo cbo_controllable = null, cbo_observable = null;
     ...
     *            The platform in which this TransitionData will exist.
     * @param parent
     *            The composite in which this TransitionData's objects will be
     *            embedded.
     */
    public TransitionData(GraphingPlatform graphing_platform, Composite parent) {
        ...
        GridData gd_edit_region = new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.VERTICAL_ALIGN_FILL | GridData.GRAB_HORIZONTAL);
        cmp_edit_region.setLayoutData(gd_edit_region); // for the composite
                                                        // within the tabfolder

        // create a layout for the edit region composite (for the widgits inside
        // the composite)
        GridLayout gl_edit_region = new GridLayout();
        gl_edit_region.verticalSpacing = 3;
        gl_edit_region.numColumns = 8;
        cmp_edit_region.setLayout(gl_edit_region); // attach it to the
                                                    // composite
        ...
        lbl_machine_code.setText(ResourceManager.getString("edge.machine_code")
                + ": ");
        GridData gd_lbl_machine_code = new GridData(
        ...
        lbl_description.setText(ResourceManager.getString("edge.description")
                + ": ");
        GridData gd_lbl_description = new GridData(
                GridData.HORIZONTAL_ALIGN_END);
        ...
        cbo_controllable = new Combo(cmp_properties, SWT.DROP_DOWN
                | SWT.READ_ONLY);
        cbo_controllable.setItems(new String[] {
                ResourceManager.getString("edge.controllable"),
                ResourceManager.getString("edge.uncontrollable") });
        cbo_controllable.select(0);

        cbo_observable = new Combo(cmp_properties, SWT.DROP_DOWN
                | SWT.READ_ONLY);
        cbo_observable.setItems(new String[] {
                ResourceManager.getString("edge.observable"),
                ResourceManager.getString("edge.unobservable") });
        cbo_observable.select(0);

        Label lbl_nothing = new Label(cmp_edit_region, SWT.NONE);
        GridData gd_lbl_nothing = new GridData(GridData.GRAB_HORIZONTAL
        ...
        save_button.addSelectionListener(new SelectionListener() {
        ...
        save_as_new_button.setText(ResourceManager
                .getString("edge.save_as_new_button"));
        GridData gd_save_as_new_button = new GridData();
        save_as_new_button.setLayoutData(gd_save_as_new_button);
        save_as_new_button.addSelectionListener(new SelectionListener() {
            public void widgetSelected(SelectionEvent e) {
                saveNewData();
            }
        ...
        delete_selected_button.setText(ResourceManager
                .getString("edge.delete_selected_button"));
        GridData gd_delete_selected_button = new GridData();
        delete_selected_button.setLayoutData(gd_delete_selected_button);
        delete_selected_button.addSelectionListener(new SelectionListener() {
            public void widgetSelected(SelectionEvent e) {
                deleteData();
            }
        ...
        GridData gd_lbl_latex = new GridData(GridData.HORIZONTAL_ALIGN_END
                | GridData.VERTICAL_ALIGN_BEGINNING);
        lbl_latex.setLayoutData(gd_lbl_latex);

        txt_latex = new Text(cmp_edit_region, SWT.BORDER | SWT.MULTI
        ...
        j2dcanvas = new J2DCanvas(cmp_edit_region, SWT.NULL, new IPaintable() {
        ...
        GridData gd_j2dcanvas = new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.VERTICAL_ALIGN_FILL | GridData.GRAB_HORIZONTAL
        ...
        GridData gd_table = new GridData(GridData.HORIZONTAL_ALIGN_FILL
                | GridData.VERTICAL_ALIGN_FILL | GridData.GRAB_HORIZONTAL
                | GridData.GRAB_VERTICAL);
        edges_table.setLayoutData(gd_table);

        edges_table.addListener(SWT.Resize, new Listener() {
            public void handleEvent(Event e) {
                resize();
            }
        });

        edges_table.addSelectionListener(new SelectionListener() {
            public void widgetSelected(SelectionEvent e) {
                loadData();
            }
        ...
        col_controllable
                .setText(ResourceManager.getString("edge.controllable"));
        ...
        col_machine_code
                .setText(ResourceManager.getString("edge.machine_code"));
        col_machine_code.pack();

        col_sizes = new int[] { col_name.getWidth(), col_latex.getWidth(),
                col_symbol.getWidth(), col_controllable.getWidth(),
                col_observable.getWidth(), col_machine_code.getWidth() };
        ...
        cmp_trace_region.setLayoutData(gd_trace_region); // for the composite
                                                            // within the
                                                            // tabfolder

        // create a layout for the trace region composite (for the widgits
        ...
        cmp_trace_region.setLayout(gl_trace_region); // attach it to the
                                                        // composite
        ...
        txt_trace.addKeyListener(new KeyListener() {
    ...
    public void draw(Graphics2D g2d) {
        if (selected_item != null && !selected_item.isDisposed()
                && selected_item.getData("awt") != null) {
            g2d.drawImage((BufferedImage) selected_item.getData("awt"), null,
                    5, 5);
        } else {
    ...
    public void resize() {
        if (gp.tabFolder.getSelectionIndex() == GraphingPlatform.SPECIFICATIONS_TAB) {
            ...
            for (int i = 0; i < col_sizes.length; i++) {
                used_width = used_width + col_sizes[i];
            }
    ...
    public void loadData() {
        ...
        cbo_controllable.select(convertToCombo(selected_item
                .getText(SPEC_CONTROLLABLE)));
        cbo_observable.select(convertToCombo(selected_item
                .getText(SPEC_OBSERVABLE)));
    ...
    /**
     * Save the data from the editable area to the last selected table item (ie
     * row)
     */
    public void saveData() {
        if (selected_item != null && !selected_item.isDisposed()) {
            ...
            selected_item.setText(SPEC_CONTROLLABLE,
                    convertFromCombo(cbo_controllable.getSelectionIndex()));
            selected_item.setText(SPEC_OBSERVABLE,
                    convertFromCombo(cbo_observable.getSelectionIndex()));
            selected_item
                    .setText(SPEC_MACHINE_CODE, txt_machine_code.getText());
    ...
    public void saveNewData() {
        if (txt_name.getText().length() + txt_latex.getText().length()
                + txt_symbol.getText().length()
                + txt_machine_code.getText().length()
    ...
    public void deleteData() {
        if (edges_table.getSelectionIndex() >= 0) {
            ImageSupport.safeDispose(edges_table.getItem(
                    edges_table.getSelectionIndex()).getData("swt"));
            edges_table.getItem(edges_table.getSelectionIndex()).setData("swt",
                    null);
    ...
    public void printData(PrintWriter out) {
        ...
        for (int i = 0; i < table_items.length; i++) {
            out.println("    " + "data" + i + "." + spec_strings[SPEC_NAME]
                    + " " + table_items[i].getText(SPEC_NAME));
            out.println("    " + "data" + i + "." + spec_strings[SPEC_LATEX]
                    + " "
                    + Ascii.escapeReturn(table_items[i].getText(SPEC_LATEX)));
            out.println("    " + "data" + i + "." + spec_strings[SPEC_SYMBOL]
                    + " " + table_items[i].getText(SPEC_SYMBOL));
            out
                    .println("    "
                            + "data"
                            + i
                            + "."
                            + spec_strings[SPEC_CONTROLLABLE]
                            + " "
                            + convertToCombo(table_items[i]
                                    .getText(SPEC_CONTROLLABLE)));
            out.println("    " + "data" + i + "."
                    + spec_strings[SPEC_OBSERVABLE] + " "
                    + convertToCombo(table_items[i].getText(SPEC_OBSERVABLE)));
            out.println("    " + "data" + i + "."
                    + spec_strings[SPEC_MACHINE_CODE] + " "
                    + table_items[i].getText(SPEC_MACHINE_CODE));
            out.println("    " + "data" + i + "."
                    + spec_strings[SPEC_DESCRIPTION] + " "
                    + table_items[i].getText(SPEC_DESCRIPTION));
        }
    }

    /**
     * Input the data from the gml format. This method empties the tokenizer.
     * 
     * @param in
     *            An initialized BufferedReader for inputing the data from file.
     * @param st
     *            The string tokenizer containing this_line.
     * @param this_token
     *            The first token from st, hence st.nextToken will return the
     *            second token.
     * @param this_line
     *            The line that st has tokenized
     * @return true if the operation was successful.
     */
    public boolean inputData(BufferedReader in, StringTokenizer st,
            String this_token, String this_line) {
        boolean success = false;
        try {
            if (this_line.length() >= 5 + this_token.length()) {
                // format is 4 spaces then the label (token1) then a space then
                // the remainder of the line is the data
                this_line = this_line.substring(5 + this_token.length());
                // the calling function needs the tokenizer to be empty else it
                // will continue processing this line
            ...
            if (this_token.endsWith("." + spec_strings[SPEC_NAME])) {
                input_item = new TableItem(edges_table, SWT.NONE); // this data
                                                                    // is for a
                                                                    // new
                                                                    // TableItem
                ...
                success = true;
            } else if (this_token.endsWith("."
                    + spec_strings[SPEC_CONTROLLABLE])) {
                input_item.setText(SPEC_CONTROLLABLE,
                        convertFromCombo(this_line));
                success = true;
            } else if (this_token.endsWith("." + spec_strings[SPEC_OBSERVABLE])) {
                input_item
                        .setText(SPEC_OBSERVABLE, convertFromCombo(this_line));
                success = true;
            } else if (this_token.endsWith("."
                    + spec_strings[SPEC_MACHINE_CODE])) {
                ...
                success = true;
                    ...
                    .endsWith("." + spec_strings[SPEC_DESCRIPTION])) {
     ...
     * label type providing that the new type has an empty value. Also calls
     * renderIfNecessary()
     */
    public void fillBlankLabels() {
        ...
        for (int i = 0; i < table_items.length; i++) {
            if (table_items[i].getText(SPEC_SYMBOL).length() < 1) {
                table_items[i].setText(SPEC_SYMBOL, table_items[i]
                        .getText(SPEC_LATEX));
            }
     ...
     * 
     * @param text_meaning
     *            The text meaning of a binary selection combo.
     * @return 0 for "Yes", 1 otherwise.
     */
    private int convertToCombo(String text_meaning) {
        if (text_meaning == BOOLEAN_COMBO_TRUE) {
            return 0;
        } else {
            return 1;
        }
    ...
    public int[] getTrace() {
        ...
        if (s.countTokens() > 0) {
            ...
            while (s.hasMoreTokens()) {
                try {
                    symbols[i] = Integer.parseInt(s.nextToken());
                } catch (Exception e) {
                    symbols[i] = -1;
                }
                i++;
    ...
    public String getTraceString() {
        return txt_trace.getText();
    }

    public void setTraceString(String new_trace) {
        txt_trace.setText(new_trace);
    }
}