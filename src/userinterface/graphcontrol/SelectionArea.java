...
/**
 * This class represents a crosshairs, and dotted line rectangle area. It is
 * used both to specify the print area and to generate groupings. The two
 * behaviours require some speeration in some of the methods, but the both have
 * a lot in common.
 * 
 * @author Michael Wood
 */
public class SelectionArea {
    ...
    /**
     * Records the location of the selection area. values:
     * {first_x,first_y,second_x,second_y,state,cursor,active_index}. state = 0 ->
     * invisible. state = 1 -> visible. state = 2 -> in mid modify (ie mouse is
     * down). cursor = 0 -> cross. cursor = 1 -> lr. cursor = 2 -> ud. cursor =
     * 3 -> hand.
     */
     ...
     * @param option
     *            Specifies the behaviour of this SelectionArea.
     */
     ...
     *            Specifies the behaviour of this SelectionArea.
     * @param coords
     *            The starting co-ordinates in the form: "x1,y1,x2,y2"
     */
    ...
    public void mouseUp() {
        if (option == MARKING_OUT_AN_AREA) {
            ...
            if (selection_area[5] == 3) {
                // moving the area
                selection_area[4] = 1; // set as visible, hence, no longer in
                                        // mid modify
            } else {
                ...
                int x1, y1, x2, y2;
                if (selection_area[0] > selection_area[2]) {
                    x2 = selection_area[0];
                    x1 = selection_area[2];
                } else {
                    x2 = selection_area[2];
                ...
                if (selection_area[1] > selection_area[3]) {
                    y2 = selection_area[1];
                    y1 = selection_area[3];
                } else {
                    y2 = selection_area[3];
    ...
    public boolean isInBounds(int x, int y) {
        if (x > selection_area[0] && x < selection_area[2]
                && y > selection_area[1] && y < selection_area[3]) {
            return true;
        } else {
            return false;
        }
    ...
    private void setVisibleBySize() {
        if (Math.abs(selection_area[0] - selection_area[2]) > 20
                && Math.abs(selection_area[1] - selection_area[3]) > 20) {
            selection_area[4] = 1;
        } // mark as "visible"
        else {
            selection_area[4] = 0;
    ...
    public void draw(Drawer drawer) {
        if (selection_area[4] > 0) {
            ...
            if (selection_area[0] < selection_area[2]) {
                x = selection_area[0];
                w = selection_area[2] - selection_area[0];
            } else {
                x = selection_area[2];
                w = selection_area[0] - selection_area[2];
            }
            if (selection_area[1] < selection_area[3]) {
                y = selection_area[1];
                h = selection_area[3] - selection_area[1];
            } else {
                y = selection_area[3];
                h = selection_area[1] - selection_area[3];
            }
