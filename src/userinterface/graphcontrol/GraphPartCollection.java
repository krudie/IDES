...
/**
 * This class keeps track of current groupings within a graph. Items can be
 * added or removed from groups individually or on mass with the selection
 * tools. This is mostly used for copy/paste/delete/movement.
 * 
 * @author Michael Wood
 */
public class GraphPartCollection{
     ...
     * there is a double-click action, it can be informed of what was
     * accomplished in the down event. The up event clears this variable
     */
     ...
     * Adding and removal should update the objects "grouped" attribute Only a
     * doubleclicked item should have the "live text" attribute, hence we must
     * remove it from any existing. Note: double clicking can never be part of a
     * ctrl grouping
     * 
     * @param object
    ...
    /**
     * If ctrl is pressed, then the grouped_parts should be updated, Else the
     * grouped_parts should be nulled, Either way, the last_grabbed_object
     * should be updated. The live_text attribute is only received on a
     * double_click.
     * 
     * @param state_mask
     ...
     * Update the last_grabbed_object to the object at the top of the list new
     * list. This is for removal of dead objects, so their attributes don't
     * matter.
     * 
     * @param object
    ...
    /**
     * Calculate a bounding area for this grouping Note: similar code is used in
    ...
    /**
     * Call exclusive intiateMovement on all Nodes in the group, so that their
    ...
    public void initiateNodeMovement(Point mouse){
        if(parts != null){
            ...
            // also, mark all selected edges as SAFE_GROUPING if both of their
            // nodes are grouped
    ...
    /**
     * Call exclusive updateMovement on all Nodes in the group, so that their
     * edges outside the group can be updated
     * 
     * @param mouse
