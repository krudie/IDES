...
/**
 * This class provides various character and String manipulation and testing methods.
 * 
 * @author Michael Wood
 */
public final class Ascii 
{
	 ...
	 */
	public static String string(int c) { return "" + (char)c; }
	...
	public static int occurrances(String s, int c)
	{
		...
		{
			if (s.charAt(i) == (char)c) { count++; }
		}
		return count;
	}
	
	/**
	 * Test if the input is a "normal" character (0x20-0x7E or CR).
	 * That is all alphanumeric and punctuation and only CR from the special characters
	 * 
	...
	{
		int i = (int)c;
		if ((i > 31 && i < 127) || i == 13) { return true; }
		return false;
	}
	
	/**
	 * Replaces all occurences of "find" in "source" with "replace".
	 * Moves left to right, so if "replace" contains "find" it will not be further modified.
	 * Use this when String.replaceAll is too complicated (i.e. replace < with $<$)
	 * 
	 * @param	source	The source string.
	 * @param	find	The substring to be found.
	 * @param	replace	The substring to replace all occurences of find.
	 * @return  A modified version of "source" will all occurences of "find" replaced with "replace".
	 */
	public static String replaceAll(String source, String find, String replace)
	{
		...
		if (find.length() > 0)
		{
			...
			while(i>=0 && i<r.length())
			{
				...
				if (i>=0)
				{
					if (i > 0) { left_half = r.substring(0,i); } else { left_half = ""; }
					if (i + find.length() < r.length()) { right_half = r.substring(i + find.length()); } else { right_half = ""; }
