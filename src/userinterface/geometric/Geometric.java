...
/**
 * This class provides various methods for geometric computation, such as the
...
public final class Geometric {
    /**
     * Calculate the angle between arm1 and arm2 through vertex. Clockwise from
     * arm1 to arm2 is positive.
     * 
     * @param arm1
     *            The Point that defines the first arm of the angle.
     * @param vertex
     *            The Point that defines the vertex of the angle.
     * @param arm2
     *            The Point that defines the second arm of the angle.
     * 
    ...
    /**
     * Calculate the angle where the first arm is from the vertex down (which is
     * the positive y direction) and the second arm is from the vertex to the
     * given arm point and positive is in the clockwise direction.
     * 
     * @param vertex
     *            The Point that defines the vertex of the angle.
     * @param arm
     *            The Point that defines the arm of the angle that is not the
     *            vertical arm.
     * 
    ...
    public static float absoluteRadians(Point vertex, Point arm) {
        ...
        if (arm.x == vertex.x) {
            ...
            a = (float) Math.atan((float) Math.abs(arm.y - vertex.y)
                    / (float) Math.abs(arm.x - vertex.x));
            if (arm.y > vertex.y && arm.x > vertex.x) {
                a = a + (float) (3 * Math.PI / 2);
    ...
    /**
     * Calculateds the distance between a point and the origin. This is also the
     * length of a vector.
     * 
     * @param x
     ...
     * @return The distance between the point and the origin (aka vector
     *         length).
     */
    ...
    public static float distance(int x1, int y1, int x2, int y2) {
        float mag = (float) Math.sqrt(Math.pow(x1 - x2, 2)
                + Math.pow(y1 - y2, 2));
        if (mag == 0) {
            mag = Float.MIN_VALUE;
        }
     ...
     * 
     * @param p
     *            A point representing a vector from the origin.
     * @param u
     *            A unit vector.
